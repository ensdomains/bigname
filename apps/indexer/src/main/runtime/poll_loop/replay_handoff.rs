...
use super::super::intake::{IntakeChainTask, clone_intake_chain_task_with_progress};
...
pub(super) async fn poll_replay_ready_chains_raw_only(
    ...
    let mut scoped_tasks = Vec::with_capacity(raw_poll_chains.len());
    {
        ...
        {
            scoped_tasks
                .push(clone_intake_chain_task_with_progress(pool, task, &mut progress).await?);
        }
    }
