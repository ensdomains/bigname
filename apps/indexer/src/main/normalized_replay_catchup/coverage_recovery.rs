...
const MAX_ENS_V2_COVERAGE_RECOVERY_ATTEMPTS: usize = 32;
...
pub(super) async fn replay_full_closure_with_coverage_recovery(
    ...
    loop {
        ...
        if recovery_attempt >= MAX_ENS_V2_COVERAGE_RECOVERY_ATTEMPTS {
            return Err(replay_error.context(format!(
                "normalized replay ENSv2 coverage recovery did not converge within {MAX_ENS_V2_COVERAGE_RECOVERY_ATTEMPTS} attempts"
            )));
        }
        let Some(provider) = provider else {
            return Err(replay_error.context(format!(
