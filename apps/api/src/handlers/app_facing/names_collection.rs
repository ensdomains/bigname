    ...
    let meta = parsed.meta.include_summary().then(|| {
        build_compact_meta(
            parsed
                .include
                .total_count
                .then_some(storage_page.total_count),
...
fn parse_names_request(query: NamesQuery) -> ApiResult<ParsedNamesRequest> {
    ...
    if matches!(view, ResponseView::Full) {
        return Err(ApiError {
            status: StatusCode::BAD_REQUEST,
            code: "invalid_input",
            message: "view=full is reserved for a later compact names implementation".to_owned(),
        });
    }
...
fn parse_names_address_filter(
    ...
    match (owner, account, registrant, relation) {
        ...
        (None, Some(account), None, relation) => {
            Ok(Some(bigname_storage::NameCurrentAddressFilter {
                address: account,
                relation: relation
