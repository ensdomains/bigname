...
pub(crate) use values::projected_registration_resource_id;
...
use values::{
    ...
    json_value_present, network, object_field, response_chain_id, served_registration_id,
};
...
pub(crate) fn build_name_record(
    ...
    Ok(NameRecord {
        registration_id: (registration.registration_status != RegistrationStatus::Unregistered)
            .then(|| served_registration_id(row))
            .flatten(),
