...
pub(crate) fn primary_name_verified_result(
    ...
    match &lookup_state.on_demand_verified {
        ...
        OnDemandPrimaryNameVerificationState::AuthorityUnsupported(reason) => {
            return json!({ "status": "unsupported", "unsupported_reason": reason });
        }
        OnDemandPrimaryNameVerificationState::Verified(result) => return result.clone(),
    ...
    match lookup_state.on_demand_claim {
        ...
        OnDemandPrimaryNameClaimState::NotAttempted
    ...
    match lookup_state.tuple_state {
        PrimaryNameTupleState::TupleMissing => json!({ "status": "not_found" }),
