...
fn is_verified_execution_request(method: &Method, uri: &Uri) -> bool {
    ...
    match segments.as_slice() {
        ...
        ["v1", "names", _, _, "records"] => {
            query_matches(uri, "mode", &["auto", "verified", "both"])
        }
        ["v2", "addresses", _, "primary-name"] => true,
        ["v2", "names", _] => query_matches(uri, "source", &["verified"]),
        ["v2", "names", _, "records"] => query_matches(uri, "source", &["auto", "verified"]),
