...
async fn born_wrapped_detail_and_history_keep_the_wrapper_registration_handle() -> Result<()> {
    ...
    seed_identity_name(&database, LOGICAL, NAME, NAME, &namehash, wrapper, Uuid::from_u128(0x8140), Uuid::from_u128(0x9140), "0x0000000000000000000000000000000000007140", bigname_storage::AddressNameRelation::Registrant, 137).await?;
    let orphan_wrapper = Uuid::from_u128(0x7143); upsert_test_resources(&database.pool, &[address_name_resource(registrar, None, "0xborn-wrap-registrar", 79), address_name_resource(rewrapper, None, "0xborn-wrap-rewrapper", 79), address_name_resource(orphan_wrapper, None, "0xborn-wrap-orphan", 79)]).await?;
    ...
    let unwrapped = v2_history_payload_for_database(&database, &format!("/v1/events?registration_id={wrapper}&page_size=20")).await?; assert_eq!(history_types(unwrapped["data"].as_array().expect("unwrapped history data")), vec!["transfer", "transfer", "authority", "registration"]);
    ...
    bigname_storage::insert_normalized_event_fixtures(&database.pool, &[rewrap_binding, rewrap_transfer, permission, record]).await?; let detail = v2_history_payload_for_database(&database, &format!("/v1/names/{NAME}")).await?; assert_eq!(detail["data"]["registration_id"], json!(wrapper.to_string()));
    let history = v2_history_payload_for_database(&database, &format!("/v1/names/{NAME}/history?scope=registration&page_size=20")).await?; let rows = history["data"].as_array().expect("history data"); let direct = v2_history_payload_for_database(&database, &format!("/v1/events?registration_id={wrapper}&page_size=20")).await?; let direct_rows = direct["data"].as_array().expect("direct history data");
    ...
    let registrar_history = v2_history_payload_for_database(&database, &format!("/v1/events?registration_id={registrar}&page_size=20")).await?; assert!(registrar_history["data"].as_array().is_some_and(Vec::is_empty), "registrar-scoped rows leaked under a second registration handle: {registrar_history:?}");
    let rewrapper_history = v2_history_payload_for_database(&database, &format!("/v1/events?registration_id={rewrapper}&page_size=20")).await?; assert!(rewrapper_history["data"].as_array().is_some_and(Vec::is_empty), "later-wrapper rows leaked under a second registration handle: {rewrapper_history:?}");
...
async fn reservation_product_rows_omit_registration_identity() -> Result<()> {
    ...
    let blocks = (120..=125)
        ...
        .collect::<Vec<_>>();
    upsert_phase_raw_blocks(&database.pool, &blocks).await?;
    let published_at = sqlx::types::time::OffsetDateTime::from_unix_timestamp(1_700_000_125)?;
    seed_schema_v2_ens_lookup_head(
        &database.pool,
        125,
        "0xhistory125",
        &crate::v2::format_timestamp(published_at),
    )
    .await?;
    seed_identity_name(
        &database,
