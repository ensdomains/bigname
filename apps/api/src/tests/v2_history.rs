...
async fn born_wrapped_detail_and_history_keep_the_wrapper_registration_handle() -> Result<()> {
    ...
    bigname_storage::insert_normalized_event_fixtures(&database.pool, &[rewrap_binding, rewrap_transfer, permission]).await?; let detail = v2_history_payload_for_database(&database, &format!("/v2/names/{NAME}")).await?; assert_eq!(detail["data"]["registration_id"], json!(wrapper.to_string()));
