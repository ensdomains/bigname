...
struct ExactNameSnapshotSelector<'a> {
    at: Option<&'a str>,
...
async fn resolve_exact_name_selected_snapshot(
    pool: &PgPool,
    namespace: &str,
...
async fn load_name_current_for_selected_snapshot(
    pool: &PgPool,
    namespace: &str,
    name: &str,
...
fn map_internal_api_error(error: ApiError, message: impl Into<String>) -> ApiError {
...
fn name_not_found_error(namespace: &str, name: &str) -> ApiError {
    ApiError {
...
async fn exact_name_snapshot_scope(
    pool: &PgPool,
    namespace: &str,
    selector: ExactNameSnapshotSelector<'_>,
    include_resolution_auxiliary: bool,
...
fn selector_mentions_ens_sepolia_profile(selector: ExactNameSnapshotSelector<'_>) -> bool {
    selector
        .chain_positions
        .or_else(|| selector.at.filter(|value| value.trim_start().starts_with('{')))
        .is_some_and(|value| value.contains("ethereum-sepolia"))
}

fn selector_mentions_ens_mainnet_profile(selector: ExactNameSnapshotSelector<'_>) -> bool {
    selector
        .chain_positions
        .or_else(|| selector.at.filter(|value| value.trim_start().starts_with('{')))
...
fn parse_exact_name_snapshot_selector(
    selector: ExactNameSnapshotSelector<'_>,
    ...
    SnapshotSelectorInput::new(at, chain_positions, consistency).map_err(snapshot_selection_api_error)
}

fn parse_snapshot_at(value: &str, scope: &SnapshotSelectionScope) -> ApiResult<SnapshotAt> {
...
fn snapshot_selection_api_error(error: SnapshotSelectionError) -> ApiError {
