...
async fn insert_cached_outcome_for_block(
    pool: &sqlx::PgPool,
    chain_id: &str,
    block_number: i64,
    block_hash: &str,
    ...
    let trace_id = format!("00000000-0000-0000-0000-{fixture_id:0>12}");
    let cache_key = format!("head-reorg-cache-{fixture_id}");
    let request_key = format!("ens:head-reorg-{fixture_id}.eth:addr:60");
    let position = json!({
        "chain_id": chain_id,
    ...
    .bind(&trace_id)
    .bind(&request_key)
    .bind(json!({ "requested_positions": [position.clone()] }).to_string())
    .bind(json!({ "manifest_versions": manifest_versions.clone() }).to_string())
    .execute(pool)
    .await?;

    sqlx::query(
        r#"
        INSERT INTO public.execution_cache_outcomes (
            execution_cache_key,
            request_key,
    ...
    Ok((cache_key, trace_id))
}

async fn insert_two_block_competing_fork(pool: &sqlx::PgPool, chain_id: &str) -> Result<()> {
    for (number, hash, parent) in [
        (1_i64, "head-cache-new-1", format!("{chain_id}-block-0")),
        (2_i64, "head-cache-new-2", "head-cache-new-1".to_owned()),
    ] {
        sqlx::query(
            "
            INSERT INTO chain_lineage (
...
async fn row_count(pool: &sqlx::PgPool, table: &str, key_column: &str, key: &str) -> Result<i64> {
    let query = format!("SELECT COUNT(*) FROM {table} WHERE {key_column} = $1");
    Ok(sqlx::query_scalar(&query).bind(key).fetch_one(pool).await?)
}

#[tokio::test]
async fn head_publication_evicts_displaced_execution_outcome_but_retains_trace() -> Result<()> {
    let scratch = ScratchDatabase::create("phase_runner_head_cache_evict").await?;
    bigname_storage::MIGRATOR.run(scratch.legacy_pool()).await?;
    let chain_id = "head-cache-evict-chain";
    seed_lineage(scratch.pool(), chain_id, 2).await?;
    publish_heads(
        scratch.pool(),
        chain_id,
    ...
    .await?;
    let (cache_key, trace_id) = insert_cached_outcome_for_block(
        scratch.pool(),
        chain_id,
        2,
        &format!("{chain_id}-block-2"),
        "1",
    )
    .await?;
    insert_two_block_competing_fork(scratch.pool(), chain_id).await?;

    publish_heads(
        scratch.pool(),
        chain_id,
        &HeadMarkers {
            latest: BlockMarker::new(2, "head-cache-new-2")?,
            safe: None,
            finalized: None,
        },
    )
    ...
    assert_eq!(
        ...
        .await?,
        0,
        "head publication must evict an outcome anchored to a displaced block"
    );
    assert_eq!(
        row_count(
            scratch.pool(),
        ...
        .await?,
        1,
        "cache eviction must retain the durable execution trace"
    );
    scratch.cleanup().await
}

#[tokio::test]
async fn canonical_recovery_does_not_resurrect_an_evicted_execution_outcome() -> Result<()> {
    let scratch = ScratchDatabase::create("phase_runner_head_cache_restore").await?;
    bigname_storage::MIGRATOR.run(scratch.legacy_pool()).await?;
    let chain_id = "head-cache-restore-chain";
    seed_lineage(scratch.pool(), chain_id, 2).await?;
    let original_heads = HeadMarkers {
        latest: BlockMarker::new(2, format!("{chain_id}-block-2"))?,
        safe: None,
        finalized: None,
    };
    publish_heads(scratch.pool(), chain_id, &original_heads).await?;
    let (cache_key, trace_id) = insert_cached_outcome_for_block(
        scratch.pool(),
        chain_id,
        2,
        &original_heads.latest.hash,
        "2",
    )
    .await?;
    insert_two_block_competing_fork(scratch.pool(), chain_id).await?;
    ...
    let recovered_state: String = sqlx::query_scalar(
        "SELECT canonicality_state::text FROM chain_lineage WHERE chain_id = $1 AND block_hash = $2",
    )
    .bind(chain_id)
    .bind(&original_heads.latest.hash)
    .fetch_one(scratch.pool())
    .await?;
    assert_eq!(recovered_state, "canonical");
    assert_eq!(
        row_count(
            scratch.pool(),
            "public.execution_cache_outcomes",
        ...
        .await?,
        0,
        "canonical recovery must not resurrect an evicted cache row"
    );
    assert_eq!(
        row_count(
            scratch.pool(),
        ...
        .await?,
        1,
        "canonical recovery keeps the durable execution trace"
    );
    scratch.cleanup().await
}
