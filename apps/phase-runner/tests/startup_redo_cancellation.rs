//! A stop that arrives while an explicit redo is synchronizing manifests must
//! exit, and must not read as a redo that never started: manifest
//! synchronization is the first commit of the command, and whether it made it
//! is unknown once the stop wins. This drives the real binary against a held
...
async fn a_stop_during_redo_manifest_synchronization_is_not_a_redo_that_never_started() -> Result<()>
{
    ...
    let application_name = format!("startup_redo_cancellation_{}", std::process::id());
    writer
        .query_pairs_mut()
    ...
    const NOISE_KEY: i64 = 0x7374_6172_7475_7002;
    ...
    let mut child_stderr = child
        .stderr
        .take()
        .context("the runner's stderr is piped")?;
    let stderr = tokio::spawn(async move {
        let mut buffer = String::new();
        tokio::io::AsyncReadExt::read_to_string(&mut child_stderr, &mut buffer).await?;
        anyhow::Ok(buffer)
    });
    ...
    assert!(
        !status.success(),
        "a stop during manifest synchronization must not read as a redo that never started"
    );
    let stderr = stderr.await.context("collect the runner's stderr")??;
    assert!(
        stderr.contains("stop requested during manifest synchronization"),
        "{stderr}"
    );
