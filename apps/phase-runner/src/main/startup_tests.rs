...
#[test]
fn runtime_manifest_startup_admits_official_sepolia_and_preserves_source_identity() {
...
fn runtime_manifest_startup_rejects_nonzero_floor_for_other_approved_profile() {
    ...
    assert!(
        error
            .to_string()
            .contains("Sepolia intake must start at block zero")
    );
