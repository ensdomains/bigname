...
The default gate requires the exact library-test summary `93 passed; 0 failed;
...
4. Provider-fault and intake-path parity scenarios execute the real JSON-RPC
   ...
   target facts written, the redo still open and not advanced through the
   failed window, and the ingest progress seeded before the command and the
   published chain head both unchanged. A one-off truncation must be
   re-fetched by the same redo, with the re-fetch warning visible in the
   runner's log; its stored transactions, receipts, and logs on canonical
   blocks, and the normalized events and projections derived from them, are
   compared with a clean control straight away, before any later redo. A
   temporarily missing selected receipt must be refetched by the same redo;
   the test observes the retry and compares the stored rows again. A final
   clean redo is a separate idempotence check: it must leave those rows equal
   to the control.
...
`forge` must be on `PATH` before the 68 Foundry-dependent semantic scenarios are
...
scenarios plus the two zero-address and API shutdown scenarios and the eleven-log migration scenario produce 96 tests: 93 runnable and 3 ignored,
split as 46 runnable plus 2 ignored on shard 1 and 47 runnable plus 1 ignored on
...
scenarios and the eleven-log migration scenario are on shard 2. The current runnable split is 46 on shard 1 and 47 on
shard 2. Break
...
on each shard. These historical forecasts exclude the API shutdown, eleven-log migration and two zero-address scenarios; the documented family grouping is unchanged.

## Coverage ledger

The semantic inventory contains 71 scenario tests:

- 68 runnable;
- 3 explicitly retired with one-line reasons.

The 68 runnable scenarios include the #154 known-defect reproduction described
...
The crate contains 96 total tests when 25 harness/support checks are included.
The pre-retarget crate contained 88; the net change is +8: obsolete
...
resolver scenarios, two exact zero-address scenarios, and the API shutdown scenario were added. The pure in-memory
...
### Runnable semantic scenarios (68)
...
| --- | ---: | ---: | ---: |
| Total crate tests | 88 | 96 | +8 |
| Semantic scenario inventory | 62 at the retarget base, including one pure helper | 71 | -1 reclassified, -3 deleted, +13 added |
| Runnable passed-count gate | 65 in the historical Anvil gate | 93 | +28 |
| Anvil-backed semantic inventory | 65 historical gate reference | 71 | +6 |
| Runnable Anvil-backed semantic scenarios | 65 historical gate reference | 68 | +3 |

The 65 comparisons are reported because that is the historical gate reference,
but the current passed-count denominator is explicit: 68 runnable Anvil
scenarios and 25 harness/support checks produce 93 passes. Three semantic
