...
- communication-rule violations in changed docs, comments, or writeups (AGENTS.md § Communication: a bigname-specific term whose first use lacks a `docs/glossary.md` link — existing terms are linked, not re-defined inline; a new coinage gets a glossary entry plus link in the same change — or bare "promotion"/"profile" in prose; wire field names and enum values are contract, not prose)
