...
-- session search_path does not include it, so the schema name is removed before
-- comparing; runs of whitespace are collapsed to one space. The function lives
-- in pg_temp and disappears with the session.
