...
-- manual build can leave a valid index with other keys or another key order, or
...
-- 20260923130000_normalized_events_chain_block_number_desc_idx.sql. The
-- function's own SET search_path clause makes search_path pg_catalog while it
-- runs, so PostgreSQL always prints the table's schema name, and the expected
...
CREATE OR REPLACE FUNCTION pg_temp.check_events_order_index(require_built boolean)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
    ...
    IF found_kind <> 'index' THEN
        RAISE EXCEPTION
            'bigname_phase.% is a %, not an index, so the index was never built; remove or rename that relation, then follow ops/events-order-index/README.md before retrying',
    ...
    IF NOT EXISTS (
        ...
        WHERE indexrelid = to_regclass('bigname_phase.' || checked_index);
        RAISE EXCEPTION
            '% is missing from bigname_phase.normalized_events or is not valid and ready; follow the recovery steps in ops/events-order-index/README.md before retrying',
            checked_index
            USING HINT = CASE
    ...
    IF found_definition <> expected_definition THEN
        RAISE EXCEPTION
            '% exists but does not have the reviewed definition; found "%", expected "%"; follow the recovery steps in ops/events-order-index/README.md before retrying',
...
DO $$ BEGIN PERFORM pg_temp.check_events_order_index(false); END $$;

CREATE INDEX CONCURRENTLY IF NOT EXISTS normalized_events_chain_block_number_desc_idx
...
FROM pg_index
WHERE indexrelid = to_regclass('bigname_phase.normalized_events_chain_block_number_desc_idx');

-- The index must now exist, belong to bigname_phase.normalized_events, be valid
-- and ready, and have the reviewed definition.
DO $$ BEGIN PERFORM pg_temp.check_events_order_index(true); END $$;
