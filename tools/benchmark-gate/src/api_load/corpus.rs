...
mod tests {
    ...
    pub(super) async fn install_name_visibility_schema(pool: &PgPool) {
        for statement in [
            ...
            "CREATE TABLE bigname_phase.manifest_versions (manifest_id bigint GENERATED BY DEFAULT AS IDENTITY, manifest_version bigint NOT NULL DEFAULT 1, namespace text NOT NULL, rollout_status text NOT NULL, source_family text NOT NULL DEFAULT 'benchmark_non_resolver', chain_id text NOT NULL DEFAULT 'ethereum-mainnet', manifest_payload jsonb NOT NULL DEFAULT '{\"contracts\":[]}'::jsonb)",
