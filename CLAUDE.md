...
- **Bridge freshness.** Before commencing substantive work, check this binding hasn't fallen behind the canonical codex side: compare `git --no-optional-locks log -1 --format=%ci -- .codex .agents/skills` against the same for `.claude CLAUDE.md`. If the codex side is newer, reconcile first: re-port changed `.codex/agents/*.toml` into `.claude/agents/` (preserving the read-only hook wiring on `verification-reviewer`), add a `.claude/skills/` symlink and a routing-table row below for any new skill, and remove ports or symlinks whose codex counterpart is gone. Keep the reconciliation out of the task's changeset — commit it separately, and if a codex-side change is semantic rather than mechanical, report it instead of guessing.
