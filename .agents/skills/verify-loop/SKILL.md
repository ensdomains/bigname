...
5. If the reviewer reports findings, triage each one locally:
   ...
   - Treat communication-rule violations in changed docs or comments as findings: a bigname-specific term whose first use lacks a `docs/glossary.md` link (existing terms are linked, not re-defined inline; a new coinage gets a glossary entry plus link in the same change), or bare "promotion"/"profile" in prose (wire field names and enum values are exempt), fails AGENTS.md § Communication.
