-- Preserve the bounded Project scope that an entry-creating registry event
-- supplied before Interpret deletes a redo range. Fresh schemas receive the
-- same object from the phase baseline.
DO $migration$
BEGIN
    ...
    CREATE INDEX IF NOT EXISTS project_redo_child_registration_history_range_idx
        ON bigname_phase.project_redo_child_registration_history (chain_id, block_number);

    COMMENT ON TABLE bigname_phase.project_redo_child_registration_history IS
