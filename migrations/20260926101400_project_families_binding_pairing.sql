...
COMMENT ON COLUMN bigname_phase.project_binding_candidate.event_identity IS
    'This value is the event identity of the binding''s position: the position of the SurfaceBound that opened it (the block''s SurfaceBound of the same name and resource at the transaction and log index of the binding''s provenance), else the binding''s own block and provenance index with the identity binding:<surface_binding_id>. It is the final tiebreak of the canonical event order, compared as bytes; two bindings one event opened are ordered by surface_binding_id. The pairing is an assumed adapter precondition: the adapter writes a log-sourced binding and its SurfaceBound from one raw log with that log''s provenance (adapters schema_v2/identity.rs), and a block-boundary binding and its SurfaceBound from one block with no transaction or log. A binding:<surface_binding_id> identity is therefore taken to mean the adapter''s reconcile dropped the SurfaceBound, not that the SurfaceBound sits at another position.'
$ddl$;
END
$migration$;
