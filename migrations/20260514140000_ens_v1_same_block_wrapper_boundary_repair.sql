-- Repair ENSv1 same-block wrapper-boundary rows written on the wrapper
-- resource even though their log_index precedes the wrapper anchoring log.

CREATE TEMP TABLE ens_v1_same_block_wrapper_boundary_repointed_events AS
...
JOIN public.resources current_resource
 ...
 AND current_resource.provenance->>'authority_kind' = 'wrapper'
...
WHERE current_binding.canonicality_state IN ('canonical', 'safe', 'finalized')
  ...
  AND current_resource.provenance->>'authority_key' ~
      '^wrapper:[^:]+:[0-9]+:0x[0-9a-fA-F]+:0x[0-9a-fA-F]+:[0-9]+$'
...
FROM ens_v1_same_block_wrapper_boundary_repointed_events repair
...
SELECT
    ...
    'canonicality_update',
    canonicality_state
FROM ens_v1_same_block_wrapper_boundary_repointed_events;
