-- An unsupported record inventory row answers no indexed record. The Rust evaluator
-- (bigname_domain::resolver_read::evaluate_indexed_record) refuses every read whose row coverage
-- is not authoritative, so the ledger's SQL derivation must compare the same refusal instead of a
-- retained entry value. Function replacement only, retaining the existing privileges; no table or
-- data changes.
DO $migration$
BEGIN
