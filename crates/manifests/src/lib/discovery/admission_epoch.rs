...
pub async fn load_discovery_admission_epoch(pool: &PgPool, chain: &str) -> Result<i64> {
    try_load_discovery_admission_epoch(pool, chain)
        .await
        .map(|epoch| epoch.unwrap_or(0))
}

/// Recorded admission epoch for a chain, preserving a missing row as unknown.
///
/// Callers that use the epoch as durable compatibility evidence must use this
/// strict form rather than treating absence as the ordinary initial epoch.
pub async fn try_load_discovery_admission_epoch(pool: &PgPool, chain: &str) -> Result<Option<i64>> {
