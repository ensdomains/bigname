...
/// Stores a registration that same-transaction reconciliation left in place of the registry
/// record it wrote. `registry_migrated` is the marker Interpret restores its handoff state from
/// (`crates/adapters/src/schema_v2/state_restore_v1_registry.rs`); the setup marker alone is not.
async fn registration(
    ...
    let mut after = json!({
        ...
        "registration_registry_setup": true,
    });
    if migrated {
        after["registry_migrated"] = json!(true);
    }
    sqlx::query(
        "INSERT INTO normalized_events (event_identity, namespace, event_kind, source_family,
             manifest_version, chain_id, block_number, block_hash, transaction_hash,
             transaction_index, log_index, derivation_kind, canonicality_state, after_state,
             consumer_visibility, migration_correlation_ids)
         VALUES ($1, 'ens', 'RegistrationGranted', 'ens_v1_registrar_l1', 1, $2, $3, $4, $5, 0,
                 0, 'ens_v1_unwrapped_authority', 'canonical', $6, 'activated',
                 ARRAY[]::text[])",
    )
    .bind(format!("{identity}:RegistrationGranted"))
    .bind(CHAIN)
    .bind(block)
    .bind(block_hash(block))
    .bind(format!("{identity}-transaction"))
...
#[tokio::test]
async fn a_registration_marked_registry_migrated_is_a_current_record() -> Result<()> {
    let (database, pool) = database("tyr17_registry_generation_marker").await?;
    let marked = surface(&pool, 1, "ens", "marked.eth", Some("ens_v1"), None).await?;
    let unmarked = surface(&pool, 2, "ens", "unmarked.eth", Some("ens_v1"), None).await?;
    activated(
        &pool,
        "marked-1",
        Ownership::new_owner("eth", "marked", "registry_old", 1),
    )
    .await?;
    activated(
        &pool,
        "unmarked-1",
        Ownership::new_owner("eth", "unmarked", "registry_old", 1),
    )
    .await?;
    registration(&pool, "marked-3", "marked.eth", 3, true).await?;
    registration(&pool, "unmarked-3", "unmarked.eth", 3, false).await?;

    project(&pool, 2).await?;
    for logical in [&marked, &unmarked] {
        assert_eq!(
            selection(&pool, logical).await?,
            (json!("ens_v1"), json!("old"), Value::Null, Value::Null),
            "{logical} before the registration"
        );
    }
    project(&pool, 4).await?;
    assert_eq!(
        selection(&pool, &marked).await?,
        (json!("ens_v1"), json!("current"), json!(3), Value::Null)
    );
    assert_eq!(
        selection(&pool, &unmarked).await?,
        (json!("ens_v1"), json!("old"), Value::Null, Value::Null)
    );
    database.cleanup().await
}
