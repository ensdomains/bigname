...
async fn seed_basenames_resolver_manifest(pool: &PgPool) -> Result<()> {
    let payload = json!({
    ...
    let manifest_id: i64 = sqlx::query_scalar(
        "INSERT INTO manifest_versions (
         ...
         ) VALUES (1, 'ens', 'basenames_base_resolver', $1, 'test', 'active',
                   'test', 'test/basenames-resolver.toml', $2)
    ...
    sqlx::query(
        "INSERT INTO normalized_events (
             ...
             'manifest:basenames-ownerless-records', 'ens', 'SourceManifestUpdated',
             'basenames_base_resolver', 1, $1, $2, 'manifest_sync', 'canonical', $3
         )",
    )
    .bind(manifest_id)
    .bind(CHAIN)
...
#[tokio::test]
async fn basenames_ownerless_node_records_refresh_serving_inventory() -> Result<()> {
    let (database, pool) = migrated_pool().await?;
    seed_chain(&pool).await?;
    seed_basenames_resolver_manifest(&pool).await?;
    seed_surface(
        &pool,
        OWNERLESS_NAMEHASH,
        "ownerless-basename.eth",
        OWNERLESS_RESOURCE,
        OWNERLESS_BINDING,
    )
    .await?;
    ...
    seed_normalized_event(
        &pool,
        "fixture:basenames-ownerless-record-initial",
        Some(OWNERLESS_LOGICAL),
        None,
        "RecordChanged",
    ...
    seed_normalized_event(
        &pool,
        "fixture:basenames-ownerless-zero",
        None,
        Some(OWNERLESS_RESOURCE),
        "AuthorityTransferred",
        "basenames_base_registry",
    ...
    seed_normalized_event(
        &pool,
        "fixture:basenames-ownerless-record-later",
        None,
        None,
        "RecordChanged",
        "basenames_base_resolver",
    ...
    .fetch_one(&pool)
    .await?;
    assert_eq!(incremental_value, "updated basename value");
    let incremental = ownerless_serving_projection_snapshot(&pool).await?;
    run_project(&pool, 10, 8, None).await?;
    assert_eq!(
        incremental,
        ownerless_serving_projection_snapshot(&pool).await?,
        "Basenames ownerless record attribution diverged between incremental and fresh Project"
    );
