    ...
    let rows: Vec<(String, String, String)> = sqlx::query_as(
        "/* project:families.derived.touched */ WITH journal AS (
             ...
             WHERE chain_id = $1 AND block_number = $2
               AND family IN ('project_address_name_fold', 'project_node_record_value',
         ...
         SELECT DISTINCT kind, first, second FROM (
             ...
             FROM journal WHERE family = 'project_address_name_fold'
             UNION ALL
...
/// The three relations of address_names.rs `relations`, unmasked: the registrant, the token
/// holder falling back to the registrant, and the controller falling back to both.
const NAME_INSERT: &str = "/* project:families.derived.name_insert */
    ...
    SELECT DISTINCT relation.address, fold.logical_name_id, relation.relation, fold.chain_id
    FROM project_address_name_fold fold
    CROSS JOIN LATERAL (VALUES
        (fold.registrant, 'registrant'),
        (COALESCE(fold.token_holder, fold.registrant), 'token_holder'),
        (COALESCE(fold.controller, fold.token_holder, fold.registrant), 'effective_controller')
    ) relation (address, relation)
    WHERE fold.chain_id = $1 AND fold.logical_name_id = ANY($2::text[])
      AND relation.address IS NOT NULL AND btrim(relation.address) <> ''
      AND relation.address <> '0x0000000000000000000000000000000000000000'";
...
/// partition's latest version change (address_records.rs `record_entries`).
const NODE_INSERT: &str = "/* project:families.derived.node_insert */
    ...
    FROM (
        SELECT record.resolver_address, record.node, record.arm, record.arm_identity,
               ...
               lower(CASE WHEN jsonb_typeof(record.value) = 'string' THEN record.value #>> '{}'
