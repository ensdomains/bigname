...
impl Run<'_> {
    ...
    async fn follow(
        ...
        let next = family.current.as_ref().map_or(0, |marker| marker.number + 1);
        let Some(revision) = token.revision() else {
            return Err(block::revision_error(self.chain_id, next, None, &(None, None)));
        };
    ...
    async fn apply_blocks(
        ...
        let from = family.current.as_ref().map_or(0, |marker| marker.number + 1);
    ...
    async fn resume(
        ...
        match record.state {
            ...
            State::Replaying => {
                ...
                if stands {
                    return self.replay(family.clone(), attempt, &revision, outcome).await;
                }
    ...
    async fn reset(
        ...
        let mut transaction =
            self.pool.begin().await.map_err(|error| {
                ProjectError::database("failed to begin a family reset", error)
            })?;
    ...
    async fn transition(
        ...
        repair::require_state(self.chain_id, current.as_ref(), record.attempt, record.state)?;
        match transition {
        ...
        transaction.commit().await.map_err(|error| {
            ProjectError::database("failed to commit a repair transition", error)
        })
    }
}
