...
/// not move it. Today's row can instead follow the serving pointer when the name has a distinct
/// serving resource whose pointer wins the served order, or when the selected resource has no
/// pointer; this proxy follows the selected one (see `shims.rs`). The eligibility predicates, the
/// `resolver_current` gate and the returned rows are served `name_current` and `resolver_current`
/// reads, also listed there.
