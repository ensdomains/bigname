...
async fn load_projection_apply_cursors(
    ...
    let rows = sqlx::query(
        r#"
        SELECT
            projection_apply_cursors.cursor_name,
            projection_apply_cursors.last_change_id,
            max_change.max_change_id
        FROM projection_apply_cursors
        CROSS JOIN (
            SELECT MAX(change_id) AS max_change_id FROM projection_normalized_event_changes
        ) max_change
        ORDER BY projection_apply_cursors.cursor_name
        "#,
    )
    .fetch_all(pool)
    .await
    ...
    rows.into_iter()
        .map(|row| {
            Ok(ProjectionApplyCursorRow {
                ...
                max_change_id: crate::sql_row::get(&row, "max_change_id")?,
            })
        })
        .collect()
}
