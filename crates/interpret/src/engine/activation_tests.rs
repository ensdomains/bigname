...
async fn checked_in_sepolia_manifests_materialize_exactly_one_transition_predecessor() -> TestResult
{
    ...
    // integration exercises the identity writer, so leave the separately tested diagnostic
    // persistence out of this write.
...
async fn database() -> TestResult<TestDatabase> {
    let database =
        TestDatabase::create(TestDatabaseConfig::new("interpret_sepolia_activation")).await?;
    for statement in [
