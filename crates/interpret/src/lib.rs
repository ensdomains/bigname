...
pub use engine::{
    BatchOutcome, BatchRequest, DEFAULT_INTERPRETER_STATE_CACHE_ENTRIES, Engine, FullStateReason,
    Marker, RunMode, StateLoader,
};
...
mod tests {
    use bigname_adapters::schema_v2::seam::{
        ADMISSION_DISCOVERY_EDGE_KINDS, ARM_WIDE_BINDING_CLOSE_KEY, BINDING_CLOSE_CLAMP_SQL,
        CHILD_CLEANUP_EVENT_KINDS, CLOSED_AUTHORITY_ARM_KEY, EVENT_CLOSE_TIME_SQL,
        INTERPRETER_STATE_KEY, LOG_INDEX_KEY, MIGRATION_APPLIED_EVENT_KIND, OBSERVATION_KEY,
        PREIMAGE_OBSERVATION_EVENT_KIND, PROVENANCE_KIND_KEY, RAW_BLOCK_PROVENANCE_KIND,
        REDO_ARM_WIDE_CLOSE_SQL, REDO_BINDING_CLOSE_CLAMP_SQL, REDO_CLOSED_ARM_SQL,
    ...
    #[test]
    fn redo_arm_wide_close_requires_the_non_lifecycle_marker_and_exact_identity_evidence() {
        assert!(REDO_ARM_WIDE_CLOSE_SQL.contains(PREIMAGE_OBSERVATION_EVENT_KIND));
        assert!(REDO_ARM_WIDE_CLOSE_SQL.contains(ARM_WIDE_BINDING_CLOSE_KEY));
        assert!(REDO_ARM_WIDE_CLOSE_SQL.contains(CLOSED_AUTHORITY_ARM_KEY));
        assert!(REDO_ARM_WIDE_CLOSE_SQL.contains(SURFACE_BINDING_ID_KEY));
        assert!(REDO_CLOSED_ARM_SQL.contains(ARM_WIDE_BINDING_CLOSE_KEY));
        assert!(REDO_CLOSED_ARM_SQL.contains(CLOSED_AUTHORITY_ARM_KEY));
    }
