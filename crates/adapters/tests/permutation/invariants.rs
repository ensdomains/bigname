...
fn absorb_discovered_admissions(admitted: &mut Vec<AddressAdmissionInput>, from: &BatchOutput) {
    ...
    for address in &from.contract_addresses {
        ...
        // statically pre-admitted (all of ENSv1's, and ENSv2's role addresses), or, like the
        // announced registry, matched emitter-agnostically on an event that admits regardless of
        // emitter and never emits again, or the created resolver (`pool_v2::created_resolver`),
        // whose later writes the carry does decide but whose `ResolverCreated` self edge nothing
        // closes. With a pointer-move action the over-admission above would read a dropped-log
