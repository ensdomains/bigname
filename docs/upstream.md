...
> **Subgraph `isMigrated` reused for the ENSv1→ENSv2 migration on the compatibility GraphQL surface** — upstream `isMigrated` records whether a domain's ownership was established on the 2019 ENS registry rather than the original 2017 registry; bigname's `DomainFilter.isMigrated` instead restricts to names whose declared registration authority is the ENSv2 registry, and only the `true` value applies a predicate. bigname serves no `Domain.isMigrated` field, so the divergence is confined to the filter argument.
> **Upstream**: the subgraph declares `isMigrated: Boolean!` as "whether the domain has been migrated to a new registrar" `(upstream: .refs/ens_subgraph/schema.graphql:L25 @ ens_subgraph@723f1b6)`. Its value is set from the handler that observed the `NewOwner` log `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L134 @ ens_subgraph@723f1b6)`: the current-registry handler passes `true` `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L230 @ ens_subgraph@723f1b6)` `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L231 @ ens_subgraph@723f1b6)` and the old-registry handler passes `false` `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L234 @ ens_subgraph@723f1b6)` `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L238 @ ens_subgraph@723f1b6)` `(upstream: .refs/ens_subgraph/src/ensRegistry.ts:L239 @ ens_subgraph@723f1b6)`. The two registries are the 2019 deployment `(upstream: .refs/ens_subgraph/subgraph.yaml:L13 @ ens_subgraph@723f1b6)` and the 2017 deployment `(upstream: .refs/ens_subgraph/subgraph.yaml:L42 @ ens_subgraph@723f1b6)`.
> **Our rule**: `docs/architecture.md` § Subgraph-compatible GraphQL surface and `docs/consumer-capabilities.md` § GraphQL compatibility.
> **Why**: bigname does not model the 2019 ENS registry migration as a per-name flag, so the upstream question has no answer to give. Reusing the argument name keeps subgraph-shaped variables validating, and restricting the mapping to `true` keeps the surface from asserting a confident answer to a question it is not computing — a negated ENSv2-authority test would return a precise answer to the wrong question. Consumers needing the 2019 boundary should read registry authority directly rather than this filter.
> **Since**: `2026-09-01`
...
> **Later-transaction ENSv1 wrapping retains the registrar registrant until a later wrapper transfer** — upstream `wrapETH2LD` mints the wrapped token to the requested owner and emits `NameWrapped` during the wrap transaction, but bigname keeps the pre-wrap registrar holder as the served registration registrant for a registrar name wrapped in a later transaction. A subsequent ERC-1155 wrapper transfer advances the served registrant and token holder normally; wrapper authority and registrar-derived expiry remain visible throughout.
> **Upstream**: NameWrapper transfers the registrar token into wrapper custody, calls `_wrap` with the requested owner, and emits `NameWrapped`; ERC1155Fuse mints with a zero-address `TransferSingle`. `(upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L240-L278 @ ens_v1@91c966f)` `(upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L894-L903 @ ens_v1@91c966f)` `(upstream: .refs/ens_v1/contracts/wrapper/ERC1155Fuse.sol:L250-L258 @ ens_v1@91c966f)`
> **Our rule**: `docs/api-v2.md` § Naming Dictionary, `docs/consumer-capabilities.md` § Capability mapping, and `docs/projections.md` § Name surface and declared state.
> **Why**: the later wrapper binding proves which earlier controller-free registrar lifecycle became name-addressable. Keeping its pre-wrap holder on the registration projection avoids replacing that lifecycle's registrant with wrapper-custody transfer state; the first later wrapper-token transfer supplies the new user-facing holder.
> **Since**: `2026-09-04`
...
> **Sepolia receipt-backed registrar controllers remain unadmitted under the [plaintext-gap disposition](glossary.md#plaintext-gap-disposition)** — bigname does not admit `LegacyETHRegistrarController` at `0x7e02892cfc2Bfd53a75275451d73cF620e793fc0` from block `3790197` or `ETHRegistrarController` at `0xfb3cE5D01e0f33f41DbB39035dB9745962F1f968` from block `8579988`.
...
> **Our rule / why**: `docs/manifests.md` § ENSv1 (`sepolia` deployment profile) adopts #515 option (b). Admitting one or both receipt-backed controllers would only partially widen label-bearing intake and would not cover the separately deployed `WrappedETHRegistrarController` path that exposes the plaintext gap. Numeric BaseRegistrar events already establish registrar resource identity and expiry; a later controller capability slice owns any wider plaintext admission.
> **Since**: `2026-08-20`
...
> **Our rule / why**: `docs/manifests.md` § ENSv1 (`sepolia` deployment profile) records the resulting numeric-only registrar plaintext gap and defers controller admission. The address record is authoritative for correlation, but neither it nor the two cross-check-only indexer references supplies direct deployment provenance for the inclusive historical floor needed to construct a watch plan. bigname deliberately declines to generalize the BaseRegistrar's separately recorded reference-indexer exception.
> **Since**: `2026-08-20`
