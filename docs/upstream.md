...
> **Our rule**: `docs/manifests.md` § ENSv1 Phase 4 NameWrapper and PublicResolver admission plus § Capability Policy; mirrored in `docs/storage.md` § Table Families And Write Ownership, `docs/projections.md` § Resolution, `docs/api-v1.md` § `GET /v1/resolutions/{namespace}/{name}` and § `GET /v1/resolvers/{chain_id}/{resolver_address}`, and `docs/consumer-capabilities.md` § Current Status.
> **Why**: prevent false consumer-replacement claims for ENSv1 names whose current resolver is merely registry-observed, whose dynamic resolver profile is unknown, or whose older known generation lacks the requested interface, while still preserving retained generic resolver events as observed facts. Unknown dynamic resolvers and unsupported legacy interfaces stay explicit `pending` or `unsupported` profile state for profile-gated behavior until later doc-first profile admission.
> **Since**: `2026-04-21`
...
> **Why**: prevent false consumer-replacement claims for Basenames names whose current Base resolver is merely registry-observed or whose dynamic resolver profile is unknown; unknown dynamic Base resolvers stay explicit `pending` or `unsupported` profile state until later doc-first profile admission. The gate is intentionally separate from ENSv1 PublicResolver-generation profile admission and from Basenames L1 transport / execution.
> **Since**: `2026-04-22`
