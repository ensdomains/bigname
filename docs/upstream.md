...
> **ENSv1 dynamic resolver discovery deferred until follow-on implementation** — bigname currently has the static PublicResolver seed, but the frozen consumer-replacement contract now requires resolver discovery from ENSv1 registry `NewResolver` observations before declared record reads can claim complete ENSv1 resolver coverage.
> **Upstream**: `(upstream: .refs/ens_v1/contracts/registry/ENS.sol:L12 @ ens_v1@91c966f)` `(upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L89 @ ens_v1@91c966f)` `(upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L174 @ ens_v1@91c966f)`
> **Our rule**: `docs/manifests.md` § ENSv1 Phase 4 NameWrapper and PublicResolver admission, `docs/chain-intake.md` § ENSv1 And Basenames Resolver Discovery Boundary, and `docs/development-plan.md` § Phase 4: ENSv1 Adapter Slice.
> **Why**: prevent false consumer-replacement claims for ENSv1 names whose current resolver is outside the statically admitted PublicResolver or lacks supported profile admission; resolver-local record facts must come only from direct manifest admission or resolver discovery edges plus supported resolver-profile admission.
> **Since**: `2026-04-21`
