...
> **ENSv1 authority for a `.eth` name without an ENSv2 entry** — bigname follows the chain for ENSv1 and ENSv2 name authority: a current ENSv2 registration decides, a premigration reservation defers to ENSv1, and a name without a live ENSv2 entry is decided by a live ENSv1 registration, or by its latest lifecycle fact when neither arm holds it. For a name ENSv1 decides the ENSv2 Universal Resolver answers nothing: it reads only ENSv2 registries, keeps the nearest ancestor's resolver when a label has no live entry, and the deployment registers `eth` without a resolver, so the lookup fails with `ResolverNotFound`. bigname instead serves the name from its live ENSv1 registration, or as the released ENSv1 registration when that has ended.
