...
the connection stamp and transaction checks is *fence-aware*. A lower or
missing stamp cannot commit protected writes. A fence-aware worker treats that
refusal as process-fatal and exits; a binary from before the fence existed
cannot acquire a stamp and remains blocked, but may keep retrying until its
operator or supervisor replaces it.
