...
- ENSv2 automatic startup may run a bounded multi-pass fixed point after its initial raw-fact bootstrap. Full adapter reconciliation can admit a previously unknown authoritative registry interval and return the typed missing-current-generation-coverage condition; startup then reloads finite-known-start ENSv2 root, registry, and resolver targets, backfills them through the same latched finalized head, and retries reconciliation while the authoritative target set makes progress. Repeated missing coverage without discovery progress, retention-generation churn beyond its bounded retry allowance, or the discovery-expansion safety cap fails startup. Only the typed newly-required-coverage condition is recoverable; unrelated adapter failures stop immediately. Explicit raw-only mode remains adapter-free and does not run this fixed point. ENSv1 generic resolver and Basenames recursive registry scans use their separately documented intake mechanisms; this ENSv2 loop does not generalize fixed-point discovery to those families.
- ENSv2 live polling uses the same typed condition for a narrower tail-convergence loop. If an unchanged poll admits a finite-known-start root or registry interval that was not in the poll's original selected-address set, the indexer gives that interval no coverage merely because its lineage is stored. It resolves the exact historical watched-target identity, runs a current-generation raw-only hash-pinned provider backfill for only the missing interval, and retries the unchanged poll. A retention-generation or discovery-admission-epoch change before or during provider work discards the stale plan and replans; unrelated failures stop immediately. The loop is bounded to 32 recovery attempts per chain poll.
