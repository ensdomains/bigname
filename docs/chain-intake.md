...
Chain intake is canonical-chain reconciliation with a fact log attached. Subscriptions, filters, and provider notifications are latency hints; raw facts are append-only; [canonicality](glossary.md) and head [checkpoint promotion](glossary.md) are explicit state. Block hash is identity; block number is position. Live ingestion and backfill share one downstream pipeline.[^bigname-canonicality-source]

A deployment selects one [deployment profile](glossary.md) at a time. Mainnet and Sepolia facts do not share a canonical corpus, checkpoints, or [projection](glossary.md) state. The default deployment profile selects `manifests/mainnet/`; the ENSv2 Sepolia profile selects `manifests/sepolia/`. A runtime must not load both profile roots into the same intake runtime, [watch plan](glossary.md), discovery graph, or projection set.[^bigname-profile-source]
...
The correctness-critical core of intake covers durable replay facts and cache metadata for the raw-fact, payload-cache, and execution handoff boundary:[^bigname-storage-scope]

- blocks and lineage metadata
- selected and admitted target logs
- transaction, receipt, and block fields needed to decode selected logs or rebuild retained [normalized events](glossary.md) and execution outputs
...
## ENSv1 and Basenames resolver intake

ENSv1 old-registry intake is migration-aware historical [admission](glossary.md), not a second current-registry stream. `ENSRegistryOld` stays under `ens_v1_registry_l1` as an allow-listed migration-epoch input at `0x314159265dd8dbb310642f98f50c066173c1259b` with `start_block = 3327417`. The current registry `startBlock: 9380380` is the current registry's pinned start, not original ENS history.[^subgraph-l10][^subgraph-l15][^subgraph-l39][^subgraph-l42][^subgraph-l44]

Old-registry raw facts retain their emitter identity and pass a migration guard before they normalize into current topology. A current-registry `NewOwner` marks the affected subnode migrated; later old-registry `NewOwner`, `Transfer`, `NewTTL`, and non-root `NewResolver` observations for that node are retained as facts but do not overwrite the current owner, resolver, TTL, or projection input. The root resolver is the single exception: old-registry `NewResolver(ROOT_NODE, resolver)` may still update the root resolver binding. The pinned subgraph's old-registry handlers encode the same migrated-node guard and root-resolver exception.[^subgraph-ts-l134][^subgraph-ts-l230][^subgraph-ts-l238][^subgraph-ts-l246][^subgraph-ts-l252][^subgraph-ts-l259]

`NewResolver(node, resolver)` updates the node-to-resolver pointer in ENSv1 and Basenames. It does not admit the resolver address or create a discovery edge.[^v1-ens-l12][^v1-ensreg-l89][^v1-ensreg-l174][^bn-registry-l19][^bn-registry-l132][^bn-registry-l223]

Resolver-local record and version events are selected topic-first across every emitter by the active ENSv1 or Basenames resolver manifest's ENS-specific signature set. For ENSv1, a decoded event for a known node produces history even when the emitting resolver is not the node's current resolver. Record projection still uses the node's current resolver pointer, so unrelated resolver history does not become visible current state. Basenames match-all raw facts continue through the separate resolver-profile gate described below. A raw log whose payload cannot ABI-decode to the declared resolver event shape remains a raw fact but produces no normalized record event.[^v1-iaddr-l6][^v1-iname-resolver-l5][^v1-itext-l5][^v1-itext-l10]

[Resolver-profile](glossary.md) admission gates complete record-family coverage, resolver-overview completeness, resolver-local authorization semantics, latest-only behavior, and event-to-onchain-call parity.[^v1-iaddr2-l6][^v1-iaddress-l6][^v1-itext2-l5][^v1-iversionable-l5] Basenames resolver-local record, permission, alias, and resolver-overview facts remain governed by the separate `L2Resolver`-compatible profile gate.[^bn-l2resolver-l22][^bigname-discovery-source]

The first dynamic resolver-profile admission for ENSv1 is limited to ENS Labs PublicResolver-generation profiles for the relevant complete fact families. The resolver-profile gate may use direct manifest admission, first-party known-resolver admission, stored code-hash observations, proxy/implementation edges, or another explicit non-schema admission rule. Registry `NewResolver` observation alone is not enough. Unknown dynamic resolvers keep explicit `pending` or `unsupported` resolver-profile state; older admitted generations expose only the families listed for their resolver profile. PublicResolver-generation compatibility anchors to the upstream PublicResolver mixins, ERC165 support, and `ResolverBase` record-versioning.[^v1-pres-l20][^v1-pres-l31][^v1-pres-l131][^v1-pres-l150][^v1-resolverbase-l17][^v1-resolverbase-l21][^v1-resolverbase-l22][^v1-resolverbase-l23] ENSv1 resolver-profile admission does not widen Basenames resolver-profile support.[^bn-l2resolver-l22][^bn-registry-l132]

## ENSv2 Sepolia Adapter Intake

The ENSv2 Sepolia intake starts from four admitted [source families](glossary.md): `ens_v2_root_l1`, `ens_v2_registry_l1`, `ens_v2_registrar_l1`, and `ens_v2_resolver_l1` under `manifests/sepolia/ethereum/ens/...`. Direct watched roots come from the pinned post-audit current Sepolia deployment metadata for `RootRegistry`, `ETHRegistry`, and `ETHRegistrar`. `PermissionedResolverImpl` is implementation metadata for discovered or admitted resolver instances; resolver instances enter the watch plan only through manifest admission or discovery edges.[^v2-deploy-root][^v2-deploy-ethreg][^v2-deploy-ethrc][^v2-deploy-pres]
...
- Each positive-value item in `TransferSingle` or `TransferBatch` with nonzero `from` and `to` becomes one `TokenControlTransferred`. The deployed `ETHRegistry` and `UserRegistryImpl` ABIs both expose those events. Zero-address mint and burn logs remain lifecycle evidence rather than token-control transfers; `EACRolesChanged` remains a separate permission fact and is not used to infer ownership. (upstream: .refs/ens_v2/contracts/deployments/sepolia/ETHRegistry.json:L652 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/deployments/sepolia/ETHRegistry.json:L689 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/deployments/sepolia/UserRegistryImpl.json:L723 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/deployments/sepolia/UserRegistryImpl.json:L760 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L194 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L201 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L208 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L210 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L484 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L494 @ ens_v2@48b3e2d)
- `RegistryCreated()` is matched across all emitters. It admits the emitting registry from that exact transaction/log position without a parent link. `SubregistryUpdated` remains the parent-child reachability event; `ResolverUpdated` remains resolver discovery; `ParentUpdated` remains registry topology history. Direct `PermissionedRegistry` construction emits `RegistryCreated()` first, while a `UserRegistry` proxy emits it during initialization. (upstream: .refs/ens_v2/contracts/src/registry/interfaces/IRegistryEvents.sol:L9 @ ens_v2@ccaeb58) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L113 @ ens_v2@ccaeb58) (upstream: .refs/ens_v2/contracts/src/registry/UserRegistry.sol:L43 @ ens_v2@ccaeb58) (upstream: .refs/ens_v2/contracts/src/registry/UserRegistry.sol:L47 @ ens_v2@ccaeb58)[^v2-events-l49][^v2-events-l59][^v2-events-l75]
...
Post-audit `URIUpdated`, resolver `DataChanged` and `NamedDataResource`, and ERC-1155 `ApprovalForAll` remain outside active normalized intake. `ETHRenewerV1` is not an admitted emitter — only the admitted `ETHRegistrar` supplies `NameRenewed` — and `PublicResolverV2` is not an admitted resolver profile. A registry resolver edge may still make `PublicResolverV2` a watch-only contract instance whose configured generic events are retained as normalized observations. Resolver-local projection requires that the emitter match the name's current registry binding and that an ENSv2 resolver profile be explicitly admitted; without both, no selector value or authoritative record coverage is published. A current-emitter `VersionChanged` remains eligible only as the boundary of an explicit `resolver_family_pending` row, never as record-value support. These omissions are explicit capability boundaries, not absence claims about upstream behavior.[^v2-events-uri][^v2-pres-data][^v2-deploy-renewer][^v2-deploy-public-resolver][^v2-public-resolver-discovery][^v2-public-resolver-version][^v2-iethrenewer-l21] (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L336 @ ens_v2@ccaeb58) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L341 @ ens_v2@ccaeb58)
...
At most one provider source may be configured for a chain. JSON-RPC is the portable source. A Reth DB source is an optional intake source, not a protocol adapter: ENS and Basenames adapters still consume bigname [raw facts](glossary.md#raw-facts) and append adapter-owned [normalized events](glossary.md#normalized-event). The checked-in Reth DB reader targets Ethereum Mainnet Reth datadirs; other chains fail closed until they have a chain-specific reader. A Reth-backed reader satisfies the same block-hash-first provider contract as JSON-RPC for heads, exact block payloads, selected logs, receipts, and [block-anchored code observations](glossary.md#raw-facts). Live execution, trace production, and fresh API-time reads remain outside this source boundary.[^bigname-deployment-provider-source][^reth-readme-l23]

For historical state at block `B`, Reth reconstructs account state from the changeset boundary `B + 1`; an unavailable account-history lookup reports the typed error `StateAtBlockPruned(B + 1)`.[^reth-history-boundary-l937][^reth-history-provider-l946][^reth-account-history-pruned-l189] When `BIGNAME_INDEXER_CHAIN_RPC_CODE_FALLBACK_URLS` configures that chain, the indexer retries only matching pruned code-observation requests through the fallback JSON-RPC provider. Without that configuration, or when the fallback fails, the original provider error remains fatal. Local-store unavailability, chain inconsistency, missing safe/finalized checkpoints, unavailable exact payloads, and all non-pruning code-read failures remain fail-closed.
...
Provider availability is evaluated per selected deployment profile and per active watched chain. A Base provider is not a global startup prerequisite: an Ethereum-only deployment profile starts without a Base provider, and a deployment profile whose Base chain has no configured provider leaves Base provider-backed intake, automatic bootstrap, backfill catch-up, and live head following idle with an explicit `unavailable` / `no_provider` reason. Startup does not fail for other configured chains. A configured provider for a chain outside the selected deployment profile remains invalid; the runtime never ingests across profiles.[^bigname-deployment-provider-source]
...
The live tailer watches the active watched chain: manifest-declared targets and discovery-admitted targets alike. Because live logs are fetched by `blockHash` with no address filter and selected client-side, the watched-target set never changes the number of log fetches a block costs. It does not follow that a wide scope is free of every provider call: the [raw-code baseline](glossary.md#raw-code-baseline) issues one `eth_getCode` per watched address that still lacks a baseline code observation, so that cost does scale with the watched set. The baseline is a capped sweep, not a single pass: each chain verifies at most `BIGNAME_INDEXER_RAW_CODE_BASELINE_MAX_ADDRESSES_PER_TICK` addresses (default 2048) per poll tick behind an in-memory cursor, fetches code in batched provider rounds, and upserts per round. Fetched observations survive failures and restarts; a fresh process safely re-verifies the stored prefix from the beginning, and a multi-million-address surface never arms a whole-surface fetch inside one tick. Narrowing the live watch set would only avoid that baseline code-observation cost, at the price of silently dropping facts emitted by discovered targets, so no runtime mode narrows it.

A log is retained when its emitter is a watched target, or when it shares a transaction with a retained log. Same-transaction retention is a completeness rule for the emitting transaction; it is not admission of the sibling emitter as a watched target, and it must not be relied on to capture a discovered target's own events. A discovered emitter that never shares a transaction with a manifest-declared target is captured only because it is itself in the watch plan.

The live watch plan is refreshed from stored discovery edges, so a target admitted after process start enters the watch set without a restart. Live adapter sync writes those edges as it normalizes each block. The refresh is sentinel-gated: every transaction that mutates the watched surface bumps the owning chain's `discovery_admission_epochs` row, so a poll tick reloads the stored plan only when an epoch has moved and otherwise pays one read of a tiny table — never a per-tick scan of the whole watched surface. Re-deriving discovery edges from the whole stored raw-log corpus is a separate, heavier operation reserved for broad runtime refresh; it is not on the live poll tick.

Bootstrap backfill scope is independent of live watch scope. Bootstrap resolves each selected target into an address-filtered range scan, so its target set is a provider-cost decision (see [`deployment.md`](deployment.md)). A narrow bootstrap scope never implies a narrow live scope.
...
For `basenames_base_registry`, the transitional Coinbase SQL source-family backfill retains its manifest-ABI event-signature scan across all emitters instead of a selected-target address filter. This is an immutable operational scan identity, not registry-discovery authority: Basenames owner events do not admit registry emitters, and authority interpretation remains limited to the manifest-declared registry. The immutable source identity for this Coinbase SQL mode is the source family plus manifest ABI topic/signature plan, not any discovery-edge list. Materialization scopes selected identity and code observations to the returned log emitters for each bounded window, while retaining same-transaction sibling logs and selected transaction/receipt context through the shared raw-fact materializer. Basenames authority-family Coinbase SQL ranges (`basenames_base_registry`, `basenames_base_registrar`, and `basenames_base_resolver`) run raw-only: ordered [full-closure](glossary.md) normalized replay, not independent range-local adapter sync, owns authority normalization. Malformed or semantically irrelevant selected-topic collisions do not become authoritative discovery or projection evidence; full validation fails provider-only selected log identities, and sample validation only checks and materializes returned Coinbase SQL log blocks.[^bn-registry-l19][^bn-registry-l132][^bn-registry-l223][^bigname-backfill-source]
...
- Active watched chains without configured providers stay idle. A configured provider must return a finalized head at or below its canonical head; otherwise automatic bootstrap fails closed for that chain.
- The initial bootstrap planning snapshot covers every eligible manifest-declared target and every finite-known-start ENSv2 root, registry, or resolver discovery target already materialized when planning begins. Each range runs from its admitted start through the one provider finalized head captured once at start for that chain's startup run. The unfinalized canonical tail is excluded from bootstrap jobs and their number-keyed coverage facts; live intake owns that open-ended tail. Bootstrap does not cap the start to an arbitrary recent window. Match-all ENSv1 and Basenames resolver history uses the separate topic-scan mechanism described below.
...
- Startup and live adapter synchronization interpret the raw facts already present for the selected watch plan. They do not run an ENSv2 discovery fixed point or launch provider-backed coverage-recovery waves when interpretation discovers another registry or resolver. Historical facts for such a newly watched target require an explicit raw-only backfill followed by replay. Explicit raw-only mode remains adapter-free.
...
### Automatic full-closure recovery

When automatic normalized-event catch-up reports a typed set of missing or stale current-generation per-address coverage intervals, one catch-up iteration inspects every reported interval while allowing no more provider-backed attempts than the positive `BIGNAME_INDEXER_COVERAGE_RECOVERY_MAX_ATTEMPTS_PER_ITERATION` cap, which defaults to `4`. Each prepared interval becomes one exact raw-only backfill job for the historical watched `(source_family, address)` tuple and the reported inclusive bounds. A failed, backoff-delayed, terminal, or otherwise unpreparable interval is recorded independently and does not prevent later reported intervals from completing. The indexer uses the same watched-target resolution and provider-specific job construction as operator recovery: configured Coinbase SQL is preferred for that chain, otherwise the ordinary hash-pinned provider path is used. It never widens the address, family, or block interval, and addresses with low numeric values or otherwise unusual values follow the same path as every other watched address. This batch path handles the source families whose completeness is represented by those intervals; it does not create or repair the separate ENSv2 root/registry retained-history proof. If that proof is absent or stale after a destructive retention rotation, the transitional old runtime leaves normalized replay pending instead of launching the deleted ENSv2 recovery wave.

The logical idempotency key includes the deployment profile, chain, exact persisted job-source identity digest, exact bounds, current per-window recovery write epoch, and the maximum committed raw-log block revision inside those bounds when the job is created; job creation additionally scopes the key to the captured [retention generation](glossary.md). The chain-global raw-log revision is deliberately absent, so unrelated live-tail intake does not mint replacement jobs or replay a failed job from its start. The digest is computed after provider selection. For hash-pinned recovery it includes the manifest topic set used by final verification; for Coinbase SQL it additionally includes the provider, validation mode, and Coinbase topic plan used for stored-history evidence and gap fetches. A provider, validation, or topic-plan change therefore creates a distinct immutable job instead of colliding with the prior key. Recovery first follows the persisted failure record and also rediscovers the newest incomplete job with the same exact source identity, bounds, generation, and write epoch when a crash prevented that record from being written. It therefore resumes the same incomplete job even when that job's own writes advanced the in-range revision. A later in-range mutation that makes completed verification stale raises the exact-window maximum and receives a new revision-bound job instead of reusing completed evidence.

Retry scheduling is persisted per exact violation window and retention generation, independently of immutable job revisions. Provider failures use exponential backoff from 5 seconds through a 300-second cap and consume a cumulative 32-attempt budget. A per-job attempt watermark adds only the attempts not already charged for that immutable revision, so revisiting revision A after trying revision B does not count A twice. Reaching the budget writes a terminal, operator-facing failure with the cause and last job id; catch-up remains fail-closed but later polls issue no provider queries for that interval until an operator remediates and explicitly re-arms it. Re-arm and successful recovery each advance a durable per-window write epoch before clearing the failure, so a concurrent poll planned against older terminal or topic authority cannot recreate stale failure state or fail newly re-armed work. Re-arm resets the attempt counters, preserves the checkpoints, and rebinds every incomplete job revision for that exact window to the new epoch under each job's row lock. Before resolving a replacement provider or topic identity, the poll journals a failed epoch-bound revision whose maximum persisted attempt is ahead of its bound baseline, using that revision's per-job watermark even when its first crashed attempt has no failure row; a later eligible poll may then supersede it. Before reservation, the poll binds its chosen revision under the exact-window lock, clears the binding from any safe superseded revision, and therefore leaves exactly one revision reservation-eligible. The reservation fence then compares the current window epoch, cumulative failure count, bound job-attempt baseline, and maximum persisted job-attempt count under the same natural-key and job locks. A pre-rearm runner cannot consume an attempt from a rebound job, only one poll that observed attempt 31 can reserve attempt 32, and a superseded plan defers without querying the provider or publishing terminal state. A source family with no active event `topic0` values is terminal during preparation with cause `source_family_without_active_event_topic0` and consumes no provider attempt. After a bounded batch finishes, catch-up deliberately records an iteration failure naming completed, attempted, failed, delayed, terminal, and prepared-but-unattempted jobs and leaves its replay cursor unchanged; the next iteration reloads coverage and discovery authority before attempting replay again.

Coinbase SQL coverage-recovery jobs use [stored-history verification](glossary.md) before fetching log rows. The exact job range is divided into 131,072-block buckets. The heavy local aggregation runs outside the raw-log mutation fence: a short fenced read first latches the retention generation and revision, and a second short fenced read rejects the evidence if the block-revision ledger shows any in-range mutation during the scan. Canonical, safe, and finalized rows use the exact-emitter partial index; a separate `canonicality_state = 'observed'` branch uses the state/range index only to detect invalid selected rows. This keeps the scan address-bounded even for registrar addresses with roughly 1.3–4.3 million rows and, more importantly, does not block same-chain raw-log writers while those rows are aggregated. The indexer computes each bucket's selected canonical raw-log count and 128-bit identity fingerprint from the multiset of `(block_hash, transaction_hash, log_index)` identities. A bounded sequence of independent Coinbase SQL aggregate queries computes the same evidence over disjoint inclusive block sub-ranges, keeping every bucket index anchored to the exact job-range start. Each query first reduces the decoded and encoded log changelog rows to active selected identities, then scans transaction changelog rows once and aggregates transaction activity only for those identities. A bucket split across queries composes by adding its counts and XORing the two fingerprint halves, which is identical to aggregating that bucket in one query. `BIGNAME_INDEXER_COINBASE_SQL_EVIDENCE_WINDOW_BLOCKS` controls the static sub-window and defaults to 1,000,000 blocks. When a `400 Bad Request` reports either the query memory limit or the leaf bytes-read limit in a structured `errorMessage` or Coinbase's provider-rendered error body, the recovery runner halves the current sub-window and retries without advancing. Initial preparation and post-fetch re-verification use the same loop, share a limit of four halvings across the pass, and retain a successful reduced window for its remaining queries. Every halved retry emits one `INFO` event with the retry window, halving depth, and provider error class. Matching evidence proves identity-set equality, not equality of every stored payload field. A bucket is reusable only when count and fingerprint match and every selected local log has readable canonical lineage. This includes an empty bucket only when Coinbase independently reports no matching log; a local empty table scan or retained suffix is never absence proof by itself. A selected observed local log, or a selected canonical-state log without matching canonical lineage, fails the job. Adjacent mismatched buckets are coalesced into true gaps and fetched through the normal row path. Hash-pinned recovery does not use this Coinbase aggregate shortcut and fetches its exact requirement window from the provider.

After all Coinbase SQL true gaps are fetched, the job repeats the full independent aggregate comparison and fails if any bucket still differs. It then re-reads the complete exact local selector outside the mutation fence to compute the payload digest, reacquires the short fence, rejects any in-range mutation committed during that scan or any reused-bucket mutation newer than the planning revision, and records its selected-log count, deterministic digest, inclusive verified bounds, and raw-log input revision before releasing the fence. The fence must cover the final revision checks and record write, but it does not need to cover either multi-million-row digest scan because the per-block mutation ledger closes that time-of-check/time-of-use gap. A resumed checkpoint without a still-current final verification replays every provider-classified segment, including a prefix already crossed by the checkpoint; a current final verification may be reused after a crash between verification and range completion. Only then may normal range completion write coverage facts. A coverage fact backed by this record is usable only in the same retention generation, only when its interval is contained by the verified interval, and only while no later raw-log block revision touched that interval.

Provider gaps use the configured hash-pinned chunk size or Coinbase SQL initial/max window, evidence window, page, query-size, timeout, and rate knobs. A Coinbase SQL job's projected minimum includes one initial aggregate query per configured evidence sub-window, one row query per configured initial block window containing a true gap, and the same number of final aggregate sub-window queries when any gap exists. Adaptive window reductions after provider resource-limit errors, pagination, retries, and query-size filter-pack splits cannot be known before the source responds, so they appear in the actual request count. The indexer adds each returned query's attempts immediately, before validation or materialization, so a later failure does not hide paid work.
...
Automatic catch-up sweeps backfill jobs whose job and active child-range heartbeats are both older than one hour. It marks the abandoned claim failed, clears its lease, and leaves the existing checkpoint in place; the ordinary reservation path then reclaims that same immutable job and range. It also marks pending, reserved, or running automatic recovery jobs terminal when their captured retention generation is no longer current, including a pending job caught by rotation between creation and its generation fence. A fresh current-generation job or child-range heartbeat prevents the stale-claim sweep.
...
Every job also captures the chain's raw-log [retention generation](glossary.md) atomically at creation. Explicit/manual keys keep their caller-supplied identity semantics. Automatic bootstrap and operations catch-up derive their effective persisted key from the logical key plus that captured generation, and automatic checkpoint reuse requires the same generation. After compaction, the same selector and range therefore create new current-generation work instead of inheriting completed coverage from an older retained corpus.

Automatic bootstrap clips planned work only from a checkpoint whose parent job and child range are both completed. A failed, expired, or otherwise incomplete range remains resumability state for its original full-range idempotent job; it cannot become coverage authority for a suffix-only replacement. This lets the original parent finish and record gap-free full-range coverage facts atomically.
...
Backfill therefore never observes a watched target that emits nothing in the backfilled range. Live intake closes that gap: on each canonical head reconciliation it observes any watched address that has no stored non-orphaned code baseline, so a silent watched target acquires exactly one baseline observation from the live tailer rather than one per block from backfill. The watched-address set for that pass is the runtime watch plan, which unions manifest-declared contracts with watch-eligible active discovery edges. ENSv2 registry announcements and resolver discovery can therefore add baseline targets; topology-only `subregistry` edges do not. A database warmed by backfill alone, before the live tailer has reconciled a canonical head, has no code observation for any silent watched target; treat "every active watched target has at least one non-orphaned code observation" as a readiness condition for that database rather than an intake invariant.

Consumers read the latest non-orphaned code observation per target, ordered by block number and canonicality rank; none currently resolves the code at a caller-supplied historical block. Where such a read is added, it must resolve as the latest non-orphaned observation at or below the requested block number, interpolating the intervening blocks on read. Intervening blocks are deliberately not materialized as raw facts. A code change at an address that emits no selected log is therefore observed at that address's next observation, not at the block where the code changed. Declared and event-announced proxy upgrades are retained from ERC-1967 `Upgraded` logs as normalized contract history; manifest synchronization does not infer them from code-hash density. (upstream: .refs/basenames/lib/openzeppelin-contracts/contracts/interfaces/IERC1967.sol:L13 @ basenames@1809bbc) Ranges admitted before this scoping was applied uniformly carry one observation per selected target per block. Raw facts are immutable, so those denser rows are retained and remain valid observations; only newly admitted ranges are activity-scoped, and backfill no longer upgrades the canonicality state of a stored observation for a block in which that address was silent.

For ENSv1 and Basenames resolver events, source-scoped or per-target backfill is an operational repair and targeting mode over persisted watched targets. It is not the semantic address set for generic resolver-local event intake. The transitional bootstrap path can combine the existing ENSv1 resolver topic scan with address-scoped source families; the one-time Base resolver historical widening remains D1's separate ingest job. Live polling applies the active chain's resolver manifest ABI topic set with the all-emitter rule on both chains. Topic matches whose indexed fields or payload do not match the declared ABI remain raw facts but are not normalized record history. Replay and projection continue to distinguish normalized record observations from resolver-profile-gated complete-family and parity claims.[^v1-iaddr-l6][^v1-iaddress-l6][^v1-itext-l5][^v1-itext-l10][^v1-iversionable-l5][^bigname-discovery-source]

For Coinbase SQL `basenames_base_registry`, source-scoped repair likewise retains a source-family topic scan rather than an address-list lock. Registry-local events are topic-scanned across emitters for the bounded job range, then adapter logic attributes authority only to the manifest-declared Basenames registry. This stable scan identity is transitional operational compatibility; it does not admit owner addresses, expand a discovery graph, or change replay ownership.
...
Source-family backfill conformance for the shipped mainnet deployment profile proves selector correctness, source-identity stability, bounded lifecycle persistence, selected-target-only intake with same-transaction replay context, and replay coexistence. The conformance families are:[^bigname-backfill-source][^bigname-coverage-source]
...
- `basenames_execution` — same Ethereum Mainnet Basenames L1 Resolver as verified-resolution entrypoint selection. Conformance exercises the active v2 exact-surface transport-assisted direct-path class, including the entrypoint that routes `base.eth` through the root resolver and wildcard names through `OffchainLookup` / `resolveWithProof`; other Basenames verified [path classes](glossary.md) remain unsupported.[^bn-readme-l22][^bn-l1resolver-l154][^bn-l1resolver-l173][^bn-l1resolver-l191]
...
Source-family conformance does not promote any capability. Passing it does not add or widen a public route, change route-level coverage, promote manifest capabilities from `shadow` or `unsupported`, add a capability group, promote ENSv2 exact-name support, claim wrapper or migration history support, admit a fallback primary-name source, or change consumer-replacement meaning.[^bigname-coverage-source]
...
- Backfill completion is not proof of finality; canonical, safe, and finalized checkpoint promotion follows the lineage model.[^bigname-head-source]
...
8. Run canonical ENSv1, Basenames, and ENSv2 registry/discovery reconciliation through the winning head, including when the winning block has no selected logs. ENSv1 and Basenames scan closed historical registry-emitter intervals through that boundary and fail closed when their retained closure authority is unavailable. ENSv2 interprets the retained selected facts without launching provider recovery for newly discovered history. Post-head raw facts are not replay inputs, while an existing non-orphaned later assignment remains active.
9. Move the canonical head pointer last.
10. Promote blocks under the safe and finalized checkpoints asynchronously and monotonically.

Reconciliation never depends on ad hoc deletes or "latest row wins" semantics.[^bigname-reorg-source]

The legacy resolver-profile input queue, authority journal, and reconciliation tables remain in the transitional schema, but the indexer no longer drives them. Startup, live and reorg intake, backfill, operational catch-up, rewind, and raw-code repair do not run resolver-profile convergence or gate chain checkpoints on that queue.
...
Source-scoped live and backlog adapter passes do not perform a full-source discovery carry-forward per selected block; discovery mutation is bounded to observations touched by the selected raw facts plus any descendant branch made unreachable by those touched observations.

Ordinary ENSv2 registry live polling may reuse a bounded best-effort process-local lifecycle cache. Reuse requires the same deployment profile and chain, an exact selected non-orphaned ancestry anchor, unchanged raw-log retention generation and discovery-admission epoch, and per-block revision evidence that no later raw-log mutation touched the consumed ancestor path. A lower-identity late commit, canonicality repair, newly admitted log on that path, stale lineage anchor, retention-generation change, or discovery drift forces cold hydration from retained raw logs; a mutation confined to an unselected fork does not. An advancing cache decodes every raw log on the selected ancestor path after its anchor, so intermediate selected blocks cannot be skipped. An overweight result is used for the current sync but is not cached. Process restart and cache eviction also cold-hydrate: the adapter no longer persists a durable ENSv2 live checkpoint, publishes a retained-history completeness proof, or uses the deleted registry-specific synchronization fence. The indexer caller still holds the shared chain-scoped raw-log mutation guard across live interpretation and durable writes, so a final revision error cannot leave adapter output committed from an interleaved corpus.

Post-replay handoff is renewed before every live poll; a completed replay cursor does not grant permanent adapter ownership. For every provider-backed chain, the indexer revalidates the replay cursor's raw-log retention generation and commit revision through its inclusive target, then advances the `post_replay_live_adapter_backlog` cursor only after the selected post-target page is normalized and its consumed range is revalidated under the chain's raw-log mutation fence. Before enabling adapters for that poll, one sorted all-chain fence rechecks every replay prefix, backlog cursor, and current canonical raw-log maximum, and grants the one-poll permit while the fence is still held. A raw-only writer queued behind that fence may commit after it releases; that commit is therefore replay rewind or backlog work for the next handoff cycle, never silently covered by the previous permit. A prior cycle's permit also cannot authorize manifest-refresh adapter or discovery-convergence work before the fresh readiness check.

The process cache accelerates ordinary live polling only and is not replay authority. Source-scoped replay remains an explicit repair/backfill selector for bounded target sets, not the automatic catch-up default, and it is not deterministic state regeneration for stateful/contextual adapters unless the selected facts are closure-complete. The current runner fails closed for `stateful_closure_required` and `contextual_dependency_required` raw-fact replay unless a documented closure/dependency replay session exists.[^bigname-replay-source]

The global automatic replay cursor persists the raw-log retention generation and commit-ordered input revision accepted for its completed range. Catch-up captures the current version once while inspecting rewind work. A newer revision touching already-consumed history rewinds replay to the earliest changed block, and a generation change rewinds to the range start. Under the raw-log mutation fence, final cursor publication may advance to a newer revision only when per-block revision evidence proves that every intervening mutation is strictly after the completed replay boundary; a mutation at or before that boundary, missing evidence, or generation drift fails publication. Adapter-private checkpoint resume is not part of the current runner, so a failed stateful pass repeats from its retained closure boundary. Transaction timestamps and sequence allocation order are not rewind authority.

For ENSv2 automatic full-closure replay after raw-log retention has rotated, “retained-history metadata proves the replay boundary is complete” means an exact proof tuple, not a standalone boolean. The tuple binds the current raw-log retention generation, discovery-admission epoch, and an inclusive proven-through block for authoritative ENSv2 root and registry emitters. The transitional old runtime validates an already stored tuple but no longer creates, advances, or provider-recovers it. A missing or stale tuple leaves normalized replay pending; the generic exact-interval batch described above does not repair this ENSv2 proof. Plain startup and live adapter synchronization likewise do not provider-refetch a newly discovered target. The live process cache treats retention generation and discovery epoch only as invalidation inputs, not as absence authority.
...
Required failure drills: dropped subscription connection during a reorg; duplicate headers at the same height; missing parent gap that requires parent backfill; partial batch failures; crash and restart from the global replay cursor; crash and resume from a persisted backfill job range checkpoint; raw-fact normalized-event replay restart over the same bounded canonical selection as an upsert-only adapter resync whose selected replay facts come from persisted canonical raw facts (any explicit provider cache refill is block-hash-scoped, retained-digest-checked, fail-closed, and performs no checkpoint promotion); safe or finalized promotion lagging canonical intake.[^bigname-reorg-source][^bigname-backfill-source][^bigname-replay-source]
...
- Raw-fact normalized-event replay upserts normalized events only from persisted canonical selected replay facts without stale-row purge, projection rebuild, public API exposure, or chain/backfill checkpoint mutation. Ordinary replay and `--stateless-only` reject payload conflicts except for the field-level adapter repairs ratified in the storage contract. Every admitted repair replacement must publish through the normalized-event change journal.
...
[^reth-history-boundary-l937]: (upstream: .refs/reth/crates/storage/provider/src/providers/database/provider.rs:L937 @ reth@88505c7)
[^reth-history-provider-l946]: (upstream: .refs/reth/crates/storage/provider/src/providers/database/provider.rs:L946 @ reth@88505c7)
[^reth-account-history-pruned-l189]: (upstream: .refs/reth/crates/storage/provider/src/providers/state/historical.rs:L189 @ reth@88505c7) (upstream: .refs/reth/crates/storage/provider/src/providers/state/historical.rs:L190 @ reth@88505c7)
