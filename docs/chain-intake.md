...
- `SubregistryUpdated`, `ResolverUpdated`, and `ParentUpdated` become graph and topology events after their endpoint addresses resolve to current `contract_instance_id` values for the selected deployment profile.[^v2-events-l49][^v2-events-l59][^v2-events-l75]
...
- ENSv2 automatic startup may run a bounded multi-pass fixed point after its initial raw-fact bootstrap. Full adapter reconciliation can admit a previously unknown authoritative registry interval and return the typed missing-current-generation-coverage condition; startup then reloads finite-known-start ENSv2 root, registry, and resolver targets, backfills them through the same finalized head captured at startup, and retries reconciliation while the authoritative target set makes progress. Repeated missing coverage without discovery progress, retention-generation churn beyond its bounded retry allowance, or the discovery-expansion safety cap fails startup. Only the exact typed missing-current-generation-coverage condition is recoverable, whether reconciliation reports it immediately after admission or on a later pass or process restart after the requirement is already durable; unrelated adapter failures stop immediately. Automatic normalized-event catch-up handles that same typed exact tuple through the ordinary raw-only, hash-pinned provider recovery path and retries the unchanged replay only after recovery succeeds. Missing provider configuration, provider failure, authority drift, or recovery exhaustion leaves its replay cursor and adapter checkpoints unadvanced. Explicit raw-only mode remains adapter-free and does not run the startup fixed point. ENSv1 generic resolver and Basenames recursive registry scans use their separately documented intake mechanisms; this ENSv2 loop does not generalize fixed-point discovery to those families.
- Live polling admits two typed, generation-bound coverage conditions for a bounded recovery loop. ENSv2 may request a finite-known-start root, registry, or resolver interval that was absent from the poll's original selected-address set. Target-bounded ENSv1 or Basenames registry reconciliation may request the first uncovered historically authoritative `ens_v1_registry_l1` or `basenames_base_registry` emitter interval after a destructive raw-log retention-generation change. In either case the condition names one exact `(retention generation, source family, address, inclusive from block, inclusive to block)` tuple. The indexer resolves that historical watched-target identity, runs an ordinary raw-only, [hash-pinned](glossary.md) provider backfill for exactly those bounds, and retries the unchanged poll so reconciliation revalidates the complete closure proof before advancing a chain checkpoint or changing absence-aware discovery state. Recovery does not widen the selector or range, infer coverage from stored lineage, run adapters inside the recovery job, or admit any untyped error. A retention-generation or [discovery-admission-epoch](glossary.md) change before or during provider work discards the stale plan and replans. An absent or out-of-bounds historical target, provider failure, unrelated adapter failure, or repeated typed condition without convergence fails closed; the loop is bounded to 32 recovery attempts per chain poll.
...
8. Run complete canonical ENSv1, Basenames, and ENSv2 registry/discovery reconciliation through the winning head, including when the winning block has no selected logs. ENSv1 and Basenames scan closed historical registry-emitter intervals through that boundary so canonical descendant branches can be restored; post-head raw facts are not replay inputs, while an existing non-orphaned later assignment remains active. Remove losing-fork watch authority or fail closed when retained closure authority is unavailable.
9. Drain durable resolver-profile input changes produced by the winning raw code-hash view or by manifest/discovery authority changes. Reconcile retained resolver-emitter facts under the current profile, enqueue direct resolver and record-inventory invalidations, and acknowledge only the exact repaired generation.
10. Move the canonical head pointer last.
11. Promote blocks under the safe and finalized checkpoints asynchronously and monotonically.

Reconciliation never depends on ad hoc deletes or "latest row wins" semantics.[^bigname-reorg-source]

Resolver-profile convergence runs at startup after adapter/discovery bootstrap, after live or reorg raw-state persistence and before checkpoint publication, after an adapter-synchronized standalone backfill, at the end of every successful operations-catch-up iteration, before an operator rewind publishes its ancestor checkpoint, and after an execute-mode raw code-hash correction. Raw code-hash triggers compare the effective latest non-orphaned observation. They suppress duplicate notifications produced inside one transaction, while distinct committed writers remain generation-fenced and may safely request another pass; canonicality changes and corrections are never discarded as same-hash audit noise. An ordinary code-hash input outside current resolver-profile authority is acknowledged without adapter or projection work.

Manifest and discovery changes force the same generation-fenced queue without requiring a code-hash write. The durable authority journal keeps the last snapshot whose target diff was already queued, including removed targets, along with per-chain discovery-admission epochs and a revision. A full journal scan is accepted only between equal epoch reads; it force-enqueues the diff before advancing the snapshot by revision compare-and-set. Ordinary live blocks use only a cheap per-chain epoch guard when no discovery authority changed. A later no-op retry still runs the full diff when a prior discovery transaction committed and then returned an error. A dirty profile seed fans out to every active candidate derived from that seed; an ordinary candidate remains address-scoped. If another input arrives while repair is running, the final compare-and-set does not acknowledge the newer generation and the next pass repeats the repair. Automatic raw-only catch-up leaves this queue pending until normalized replay handoff restores adapter ownership; it does not advance live provider checkpoints around unfinished profile work.
...
The rewind path marks affected lineage, raw facts, identity rows, normalized events, projection inputs, and reusable execution-cache outcomes noncanonical or invalid before replaying the winning canonical selection. It emits key-scoped projection invalidations and block-hash-scoped execution invalidations. Raw-code canonicality changes may also queue resolver-profile convergence; the operator command must drain that queue successfully before updating any canonical, safe, or finalized checkpoint to the selected ancestor or reporting success. It does not delete durable audit facts, mutate manifests, widen backfill jobs, promote chain checkpoints, or let API handlers read raw facts directly.[^bigname-rewind-source]
...
Ordinary ENSv2 registry live polling keeps two restart accelerators for its lifecycle state: a bounded best-effort process-local cache and, when the state exceeds that process-cache weight limit, a versioned durable live checkpoint in the normalized replay checkpoint tables. It attempts process-local reuse first, then durable fallback, then complete cold hydration. Both cache tiers must pass the same proof checks: deployment profile and chain, checkpoint scope version, an exact selected non-orphaned ancestry anchor, raw-log retention generation, discovery-admission epoch, replay closure, and the commit-ordered raw-log input revision plus per-block revision evidence proving that no later mutation touched the consumed ancestor path. A lower-`raw_log_id` late commit, canonicality repair, newly admitted log on that path, stale lineage anchor, retention-generation change, discovery drift, or failed closure proof rejects either cached state and forces complete hydration; a mutation confined to an unselected fork does not, and sequence identity order is never commit order. Startup or process eviction uses a valid durable checkpoint when available rather than automatically hydrating the full path.

An advancing cached sync decodes every raw log on the exact selected ancestor path after its anchor, even when the caller names only the target block, so intermediate selected blocks cannot be skipped. Terminal unregister and regeneration transitions compact unreachable lifecycle entries, aliases, and subregistry suffixes before checkpoint publication. An overweight result is used for the current sync but is not published to the process cache. Instead, the adapter stages a `running` durable checkpoint parent and its complete item set under the ENSv2 registry/raw-log fence; the retained-history guard changes that exact checkpoint to `completed` in the same database transaction that advances the retained-history proof. Staging or finalization failure aborts the sync explicitly, and a `running` or partial checkpoint is never reusable. Raw-log deletion or supported staging compaction changes the retention generation and invalidates both cache tiers, while any non-live ENSv2 registry sync clears live cache state before rewriting adapter output. The database advisory fence serializes all ENSv2 registry sync entrypoints for one chain, and publication fails closed if raw-log revision, discovery admission, ancestry, or closure inputs change while the sync runs.
...
These caches accelerate ordinary live polling only. A durable live checkpoint is neither replay authority nor the durable full-closure boundary discussed above: it does not let source-scoped repair, automatic catch-up, or a non-live sync skip their documented closure requirements. Repair remains replayable because those live writes are deterministic upserts from retained raw facts. Source-scoped replay remains an explicit repair/backfill selector for bounded target sets, not the automatic catch-up default, and it is not deterministic state regeneration for stateful/contextual adapters unless the selected facts are closure-complete. The current runner fails closed for `stateful_closure_required` and `contextual_dependency_required` raw-fact replay unless a documented closure/dependency replay session exists.[^bigname-replay-source]

Durable ENSv1 adapter checkpoints and the global automatic replay cursor persist the raw-log retention generation and commit-ordered input revision that produced their state. Catch-up captures the current version once while inspecting rewind work. A newer revision touching already-consumed history rewinds replay to the earliest changed block, and a generation change rewinds to the range start. Under the raw-log mutation fence, final cursor publication may advance to a newer revision only when per-block revision evidence proves that every intervening mutation is strictly after the completed replay boundary; a mutation at or before that boundary, missing evidence, or generation drift fails publication. Transaction timestamps and sequence allocation order are not rewind authority.

For ENSv2, “retained-history metadata proves the raw-log replay boundary is complete” means an exact proof tuple, not a standalone boolean. The tuple binds the current raw-log retention generation, the discovery-admission epoch, and an inclusive proven-through block, and it authorizes retained-history closure for authoritative ENSv2 root and registry emitters only. A first raw-log insert leaves the chain incomplete. Delete or truncation advances the generation, clears the tuple, and invalidates every older live cache. Recovery requires completed, current-generation, gap-free coverage for authoritative ENSv2 root and registry intervals, including closed historical discovery intervals, while the concrete discovery epoch is fenced; retained semantic witnesses are a scoped consistency check, not absence proof. Before publishing an ENSv2 discovery-epoch advance, the indexer requires exact, current-generation coverage for every newly admitted root, registry, or resolver interval through the sync target. Resolver coverage is an admission fence: it prevents resolver history from being skipped when the watch set expands, but it does not make resolver logs part of the registry semantic-witness set or the root/registry retained-history proof. A shared raw-log table lock then remains held through loading, reconciliation, and persistence, and full-source discovery writers compare the expected epoch under their writer lock before deactivation. A complete live path may extend the proven-through boundary under the unchanged generation and expected epoch, which makes restart after live advance possible without weakening historical closure. The cold selected-parent walk stops at the earlier of that proven boundary and the earliest retained raw log from an authoritative ENSv2 root/registry emitter in its effective interval; unrelated source-family logs do not widen the ancestry walk, and a missing parent above that floor fails closed.
