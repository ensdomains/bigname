...
When automatic normalized-event catch-up reports a typed set of missing or stale current-generation coverage intervals, one catch-up iteration processes at most four reported intervals. Each interval becomes one exact raw-only backfill job for the historical watched `(source_family, address)` tuple and the reported inclusive bounds. The indexer uses the same watched-target resolution and provider-specific job construction as operator recovery: configured Coinbase SQL is preferred for that chain, otherwise the ordinary hash-pinned provider path is used. It never widens the address, family, or block interval, and addresses with low numeric values or otherwise unusual values follow the same path as every other watched address.

The logical idempotency key includes the deployment profile, chain, exact persisted job-source identity digest, exact bounds, and the raw-log input revision at which the violation batch was observed; job creation additionally scopes the key to the captured [retention generation](glossary.md). The digest is computed after provider selection. For hash-pinned recovery it includes the manifest topic set used by final verification; for Coinbase SQL it additionally includes the provider, validation mode, and Coinbase topic plan used for stored-history evidence and gap fetches. A provider, validation, or topic-plan change therefore creates a distinct immutable job instead of colliding with the prior key. Repeating recovery against the same input snapshot and provider identity reuses the existing job and child range. A later mutation that makes a completed verification stale receives a new revision-bound job instead of reusing completed evidence. After a bounded batch finishes, catch-up deliberately records an iteration failure naming every recovery job id and leaves its replay cursor unchanged; the next iteration reloads coverage and discovery authority before attempting replay again.

Coinbase SQL coverage-recovery jobs use [stored-history verification](glossary.md) before fetching log rows. Under the chain's raw-log mutation fence, the exact job range is divided into 131,072-block buckets. The indexer computes each bucket's selected canonical raw-log count and 128-bit identity fingerprint, while one independent Coinbase SQL aggregate query computes the same evidence over the exact address, manifest topic set, and block window. A bucket is reusable only when the count and fingerprint match and every selected local log has readable canonical lineage. This includes an empty bucket only when Coinbase independently reports no matching log; a local empty table scan or retained suffix is never absence proof by itself. A selected non-orphaned local log without matching canonical lineage fails the job. Adjacent mismatched buckets are coalesced into true gaps and fetched through the normal row path. Hash-pinned recovery does not use this Coinbase aggregate shortcut and fetches its exact requirement window from the provider.

After all Coinbase SQL true gaps are fetched, the job repeats the full independent aggregate comparison and fails if any bucket still differs. It then re-reads the complete exact local selector under the mutation fence and records its selected-log count, deterministic digest, inclusive verified bounds, and raw-log input revision. Before publication it rejects any mutation newer than the planning revision inside a reused bucket. A resumed checkpoint without a still-current final verification replays every provider-classified segment, including a prefix already crossed by the checkpoint; a current final verification may be reused after a crash between verification and range completion. Only then may normal range completion write coverage facts. A coverage fact backed by this record is usable only in the same retention generation, only when its interval is contained by the verified interval, and only while no later raw-log block revision touched that interval.

Provider gaps use the configured hash-pinned chunk size or Coinbase SQL initial/max window, page, query-size, timeout, and rate knobs. A Coinbase SQL job's projected minimum includes the initial aggregate verification query, one row query per configured initial block window containing a true gap, and a final aggregate verification query when any gap exists. Pagination, retries, and query-size filter-pack splits cannot be known before the source responds, so they appear in the actual request count. The indexer adds each returned query's attempts immediately, before validation or materialization, so a later failure does not hide paid work.
