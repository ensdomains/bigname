...
The live tailer watches the active watched chain: manifest-declared targets and discovery-admitted targets alike. Because live logs are fetched by `blockHash` with no address filter and selected client-side, the watched-target set never changes the number of log fetches a block costs. It does not follow that a wide scope is free of every provider call: the code-hash baseline pass issues one `eth_getCode` per watched address that still lacks a baseline code observation, so that cost does scale with the watched set. The baseline is a capped sweep, not a single pass: each poll tick verifies at most `BIGNAME_INDEXER_RAW_CODE_BASELINE_MAX_ADDRESSES_PER_TICK` addresses (default 2048) behind an in-memory cursor, fetches code in batched provider rounds, and upserts per round. Fetched observations survive failures and restarts; a fresh process safely re-verifies the stored prefix from the beginning, and a multi-million-address surface never arms a whole-surface fetch inside one tick. Narrowing the live watch set would only avoid that baseline code-observation cost, at the price of silently dropping facts emitted by discovered targets, so no runtime mode narrows it.
