...
(Issue #411 [source-role](../glossary.md#source-role) and ordering-based readiness contract; enforcement
lands in part 2)
...
| `GET /v2/status` | Per-chain indexing readiness: root `pending_invalidation_count`, `pending_invalidation_count_capped`, and `dead_letter_count` fields plus `chains: {<chain_id>: {latest_block, indexed_block, safe_block, finalized_block, lag_blocks, lag_seconds, network_block, network_head_observed_at, network_head_age_seconds, network_head_status, ingestion_lag_blocks, ingestion_lag_seconds, status}}`. Schema-v2 has no legacy invalidation queue or dead-letter table, so those retained root fields are `0`, `false`, and `0`. Chain positions come from `bigname_phase.chain_heads`, publication progress and lifecycle come from the `project` row in `bigname_phase.chain_phase_state`, and liveness comes from the newest per-chain `bigname_phase.service_heartbeats` row. The ratified Issue #411 decision requires a chain whose readiness policy needs verification evidence to read its `ingest` and `verify` rows and forbids `ready` unless Ingest remains completed and Verify is completed at or above its [verification-level](../glossary.md#verification-level) floor; unknown levels fail closed. Enforcement of that ordering-based readiness rule lands with Issue #411 part 2. A failed required Ingest or Verify maps to `stale`. An ordinary completed required Verify without the expected evidence also maps to `stale`; when startup settled that completed row while the chain was unconfigured, its marker maps to `degraded` until genuine completion or completed-state revalidation clears it, unless an independent failed phase or expired heartbeat already makes the chain `stale`. An incomplete required phase maps to `degraded`. An expired runner heartbeat remains `stale` while either required phase is incomplete. Chains without that required verification path keep Project-based readiness. Failed or heartbeat-stale state maps to `stale`; paused, redoing, missing-heartbeat, or content-generation-mismatched state maps to `degraded`. Running remains eligible for `ready` when a completed publication exists, its block and time lag are within the configured thresholds, and its generation is current; running without one is `degraded`. The provider fields are a timeout-bounded asynchronously refreshed cache comparison; the request performs no provider I/O. `status` here is the ops vocabulary `ready\|degraded\|stale` — the one non-result status enum, scoped to this route. |
