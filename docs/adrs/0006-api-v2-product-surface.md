...
| `GET /v2/status` | Per-chain indexing readiness: root `pending_invalidation_count`, `pending_invalidation_count_capped`, and `dead_letter_count` fields plus `chains: {<chain_id>: {latest_block, indexed_block, safe_block, finalized_block, lag_blocks, lag_seconds, network_block, network_head_observed_at, network_head_age_seconds, network_head_status, ingestion_lag_blocks, ingestion_lag_seconds, status}}`. Schema-v2 has no legacy invalidation queue or dead-letter table, so those retained root fields are `0`, `false`, and `0`. Chain positions come from `bigname_phase.chain_heads`, publication progress and lifecycle come from the `project` row in `bigname_phase.chain_phase_state`, and liveness comes from the newest per-chain `bigname_phase.service_heartbeats` row. A chain whose configured verification path trusts its intake provider also reads its `verify` row and cannot become `ready` until Verify is completed with the expected [verification-level evidence](../glossary.md#verification-level); a failed or completed-without-evidence required Verify maps to `stale`, while an incomplete required Verify maps to `degraded`. An expired runner heartbeat remains `stale` while Verify is incomplete. Chains without that required verification path keep Project-based readiness. Failed or heartbeat-stale state maps to `stale`; paused, redoing, missing-heartbeat, or content-generation-mismatched state maps to `degraded`. Running remains eligible for `ready` when a completed publication exists, its block and time lag are within the configured thresholds, and its generation is current; running without one is `degraded`. The provider fields are a timeout-bounded asynchronously refreshed cache comparison; the request performs no provider I/O. `status` here is the ops vocabulary `ready\|degraded\|stale` — the one non-result status enum, scoped to this route. |
