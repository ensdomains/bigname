...
- **A file would behave differently under sqlx than in the check.** A baseline file or schema-migration may not leave session state behind for the next file, use psql-only syntax, or change the server outside its databases. A schema-migration may not branch on who runs it or where, since the check cannot reproduce either, and may not catch every error.
