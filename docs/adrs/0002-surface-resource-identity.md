...
- a uniquely proved existing-token unwrapped ENSv1→ENSv2 migration keeps its registrar predecessor through the exact BaseRegistrar cleanup position; the same-transaction registry reclaim and Graveyard cleanup remain observations, without intervening registry-only bindings or a registrar reopening at cleanup. Reconciliation does not change ordinary divergence or the writer's exact-instance, exactly-one predecessor rule. See [storage transition semantics](../storage.md). (upstream: .refs/ens_v2/contracts/src/migration/UnlockedMigrationController.sol:L111-L119 @ ens_v2@a971bd64)
