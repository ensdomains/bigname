...
| Unwrap `alice.eth` before the lease ends | keep `logical_name_id`; close the wrapper binding; reactivate the prior registrar `resource_id` and prior registrar `token_lineage_id`; older retained state without that numeric registration anchor uses the ordered BaseRegistrar transfer fallback; the successor binding is still `declared_registry_path` |
