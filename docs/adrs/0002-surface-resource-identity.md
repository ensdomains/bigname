...
- if no prior registrar identity was materialized because the deployment profile retains numeric BaseRegistrar registration events only as migration-candidate input, the ordered `NameUnwrapped` then BaseRegistrar `Transfer` establishes the registrar `resource_id` and `token_lineage_id` at that transfer; later transfers and replay reuse them, without retroactively creating an ordinary registration row (upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L382-L395 @ ens_v1@91c966f)
