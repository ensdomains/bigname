...
- after that promotion, Basenames paths outside that frozen transport-assisted direct class remain explicit `unsupported`, and the first Basenames verified-resolution slice does not widen `GET /v1/primary-names/{address}`, which stays bootstrap-only under the separate reverse-intake / execution split (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L12 @ basenames@1809bbc) (upstream: .refs/basenames/src/L1/L1Resolver.sol:L13 @ basenames@1809bbc)
