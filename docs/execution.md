...
- claim-local provenance and verification-local provenance may both contribute to the route, but the claim-local side is exact-tuple declared provenance from the requested `primary_names_current(address, coin_type, namespace)` row and the verification-local side, when later published as `verified_primary_name.provenance`, is a strict section-local refinement under the same top-level `provenance.execution_trace_id` for that exact tuple
...
- the `verified_primary_name.provenance` invariants are frozen even though public field publication remains additive: when admitted, it reuses `Provenance` as a strict verification-local refinement for the same exact tuple, `verified_primary_name.provenance.execution_trace_id` must equal top-level `provenance.execution_trace_id`, and any `normalized_event_ids`, `raw_fact_refs`, `manifest_versions`, or `derivation_kind` it carries must narrow that same persisted verification trace
