...
| Subregistry attached, then swapped | SubregistryChanged derives for attach and swap at the parent registry, the parent resource survives the swap, automatic ENSv2 bootstrap fetches each finite-known-start discovered child registry in the same startup invocation, the new child's registration projects into the parent's children, and the replaced child's name leaves the current listing | covered_e2e(ens_v2_sepolia_post_audit_declared_matrix_end_to_end) |
...
| 7 | ENSv2 declared-state matrix | done as a matrix pass — fresh registration coverage, automatic discovered-source backfill, and live expiry/re-registration lineage are covered; the richer resolver/subregistry transition row remains backfill-only and blocked rows stay explicit in the ledger |
