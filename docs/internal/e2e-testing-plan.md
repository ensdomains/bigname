...
HTTP result. `covered_projection_only`, `covered_normalized_only`,
`covered_backfill_only`, and `covered_contract_only` deliberately stop at the
layer named by the row and are not evidence for public route behavior.
Execution rows also assert durable traces when the execution plane is
configured.

The ENSv1 harness intentionally keeps its small labelhash, namehash, and DNS
wire-encoding helpers independent from the production normalization and
identity implementation. Scenario inputs are literal, already-normalized
fixture names destined for contract calls; reusing the production helpers
would normalize first and make contract setup depend on the code the suite is
meant to test. These helpers are therefore an independent test oracle, not a
second production implementation, and must stay limited to fixture setup and
pinned against known vectors when their behavior changes.

## Verified foundations (phase 1, done)

The first end-to-end scenario (`register_eth_name`, a "walking skeleton" in
XP terms) established, on `main`-quality evidence, that:
...
the old registry (`registry_old`), the then-current resolver/subregistry
discovery rules, and the registry event ABI. Production does ingest the
registry. Those discovery rules were subsequently removed by the ratified B2
discovery semantics.
...
- `covered_projection_only(scenario)` — contract action through intake and a
  directly inspected projection; no public HTTP claim.
- `covered_normalized_only(scenario)` — contract action through retained raw
  facts and normalized events; no projection or public HTTP claim.
...
| Expire with no re-registration | two pinned facts: a running chain with no later authority pass keeps the registration last-known `active` with past expiry; the registrar defines a 90-day grace period (upstream: .refs/ens_v1/contracts/ethregistrar/BaseRegistrarImplementation.sol:L17 @ ens_v1@91c966f) and makes a name available only after expiry plus that grace period (upstream: .refs/ens_v1/contracts/ethregistrar/BaseRegistrarImplementation.sol:L101 @ ens_v1@91c966f). Once a retained canonical block passes that boundary, either later admitted activity or a restarted startup authority scan advances the derivation boundary, and `RegistrationReleased` materializes anchored to the first block after grace end, flipping exact-name to `released` with `released_at` from that event and excluding the name from the current registrant collection | covered_e2e(expire_without_reregistration_releases_and_unlists_registration) |
...
| Subname created with unrevealed label (the ENS Registry's `NewOwner` event carries a `bytes32` labelhash, not the plaintext label (upstream: .refs/ens_v1/contracts/registry/ENS.sol:L6 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L45 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L82 @ ens_v1@91c966f)) | bracketed placeholder child row; no exact-name surface minted (404) | covered_e2e(registry_driven_reads) |
...
| Label preimage revealed later | placeholder upgrades to the real name: registrar `PreimageObserved` and the `label_preimages` row derive, `children_current` re-projects the child as `later.preimage.eth` with `label_preimage` provenance and a stable node/owner, and no exact-name surface is minted — pinned at the derivation and projection layers via backfill + replay; **REVIEW POINT**: live re-ingest of a chain whose later 2LD registration reveals an existing placeholder child's label hangs the run loop before checkpoint promotion (silent async stall; catch-up replay of the same span derives fine), so API-layer reads of the upgraded child stay untestable pending the intake fix; pinned as a reproduced known defect | covered_backfill_only(label_preimage_revealed_later_upgrades_child_listing) |
| Set TTL on a node (registry, and routed via the wrapper) | — | blocked(manifest/adapter drift found while implementing: `ens_v1_registry_l1/v3.toml` declares the `NewTTL` fragment, but the adapter has no NewTTL observation/apply path — only migration-guard suppression (`crates/adapters/src/ens_v1_unwrapped_authority/migration_guard.rs`) — so the raw log is retained and nothing can derive; needs a doc-first decision or adapter change before an accurate row) |
...
| Root contract / DNS registrar TLD operations | — | blocked(the harness deploys neither `Root` nor `DNSRegistrar`, and no active family admits either emitter. Some effects are visible through admitted ENS Registry events: `Root.setSubnodeOwner` delegates to ENS (upstream: .refs/ens_v1/contracts/root/Root.sol:L27 @ ens_v1@91c966f), whose registry implementation emits `NewOwner` (upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L82 @ ens_v1@91c966f), while DNSRegistrar claim paths call ENS setters (upstream: .refs/ens_v1/contracts/dnsregistrar/DNSRegistrar.sol:L92 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/dnsregistrar/DNSRegistrar.sol:L108 @ ens_v1@91c966f). Those registry effects are incomplete evidence: `Root.lock` emits `TLDLocked` (upstream: .refs/ens_v1/contracts/root/Root.sol:L35 @ ens_v1@91c966f), and `DNSRegistrar._claim` emits `Claim` (upstream: .refs/ens_v1/contracts/dnsregistrar/DNSRegistrar.sol:L157 @ ens_v1@91c966f). Harness deployment plus an explicit admission-or-unsupported decision is required) |
...
| Renew a wrapped 2LD via the controller | controller renew touches only the registrar (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L366 @ ens_v1@91c966f) while the wrapper's separate renewal path updates stored expiry without emitting `ExpiryExtended` (upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L337 @ ens_v1@91c966f): the current-controller transaction derives `RegistrationRenewed` and no wrapper `ExpiryChanged`, onchain wrapper expiry stays stale, but exact-name expiry accurately follows the registrar renewal while the wrapper resource and lineage remain stable | covered_e2e(wrapped_renewal_tracks_registrar_expiry_without_wrapper_event) |
| Wrapped ERC1155 transfer (single and batch) | real `safeTransferFrom` and `safeBatchTransferFrom` calls derive no registry or wrapper-lifecycle events; the registration registrant follows each holder while resource and lineage stay stable, and the one `TransferBatch` raw log fans out into collision-free per-id `TokenControlTransferred` rows; exact-name wrapper effective control remains explicit `status=unsupported` rather than inferring holder powers or publishing stale pre-wrap facets | covered_e2e(wrapped_erc1155_single_and_batch_transfers_preserve_identity) |
...
| Resolver with no manifest target, [discovery edge](../glossary.md#discovery-graph--discovery-edge), or name pointer emits an ENS-specific record event | an extra PublicResolver writes an ETH address record while the registry resolver remains zero; match-all signature selection retains both `AddressChanged` and `AddrChanged` history for the name without manufacturing a discovery edge (upstream: .refs/ens_v1/contracts/resolvers/profiles/AddrResolver.sol:L59 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/resolvers/profiles/AddrResolver.sol:L61 @ ens_v1@91c966f) | covered_normalized_only(discovery_semantics::ens_v1_match_all_resolver_without_pointer_from_prefetched_raw_facts) |
...
| Claim routed through an unadmitted resolver | match-all intake retains the generic `NameChanged` as an unanchored observation. If interpretation runs before code-hash classification, the later replay may add one explicit unsupported-profile boundary. Every retained row lacks `primary_claim_source`, and no declared candidate is published | covered_e2e(unadmitted_reverse_resolver_keeps_candidate_absent) |
...
The first end-to-end scenario's harness already deploys both registries with
...
| Transition | Key assertions | Status |
| --- | --- | --- |
| Register through the v2 registrar | registrar intent links to the registry resource; identity, registration (`authority_kind=ens_v2_registry`), control, and history serve under `ethereum-sepolia`; the retained `RegistrarNameRegistered` evidence graduates fresh exact-name coverage to `full` / `authoritative` with `enumeration_basis=exact_name_profile`, so registration no longer waits for a renewal to lift shadow coverage | covered_e2e(renewal_preserves_promoted_coverage_and_registry_edges_follow) |
| Token regenerated (role change burns/mints token) | resource identity and surface binding stable, current token id updates | covered_normalized_only(ens_v2_registry_state_survives_distinct_live_polls) |
| Role bitmap grant/revoke, root vs resource scope | prefetched resource-scope facts retain both `PermissionChanged` bitmap transitions (zero→granted and granted→zero); root-scope projection coverage remains separate in `root_apex_attach_and_root_scope_roles` | covered_normalized_only(discovery_semantics::ens_v2_prefetched_discovery_and_lifecycle_semantics; root projection covered by root_apex_attach_and_root_scope_roles) |
| Subregistry attached, then swapped | with raw facts supplied before each interpreter pass, the first child registry and its registration are interpreted before the swap; afterward its parent edge is inactive, the replacement edge is active, and the replacement registry's child registration is present. This is interpretation coverage, not an in-session fetch or live-checkpoint claim | covered_normalized_only(discovery_semantics::ens_v2_prefetched_discovery_and_lifecycle_semantics) |
| Registry announces itself without a parent link | match-all `RegistryCreated` interpretation adds the emitting registry instance independently of `SubregistryUpdated`; no parent edge is inferred (upstream: .refs/ens_v2/contracts/src/registry/interfaces/IRegistryEvents.sol:L9 @ ens_v2@ccaeb58) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L113 @ ens_v2@ccaeb58) | covered_normalized_only(discovery_semantics::ens_v2_prefetched_discovery_and_lifecycle_semantics) |
...
| Shared subregistry → multiple surfaces, one resource | grouping by resource works, identities distinct | blocked(current discovery maps one registry address to one active suffix, so the same subregistry attached under multiple parents is superseded rather than represented as simultaneous surfaces; needs a doc/admission decision before an accurate e2e row can assert it) |
| Unregister → re-register | prefetched facts retain two distinct registration resource ids and token ids across the cycle; no live-checkpoint, projection, or route claim is made | covered_normalized_only(discovery_semantics::ens_v2_prefetched_discovery_and_lifecycle_semantics) |
| ENSv1→ENSv2 migration flow | — | blocked(migration controllers outside admission; doc-first change required) |
| Renew via the v2 registrar; direct registry renew | after registry expiry but within the post-audit grace period, registrar renew forwards to the registry (upstream: .refs/ens_v2/contracts/src/registrar/AbstractETHRegistrar.sol:L84 @ ens_v2@48b3e2d) deriving `RegistrationRenewed` + `ExpiryChanged` from one action; direct renew emits `ExpiryUpdated` only and rejects reduction (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L214 @ ens_v2@48b3e2d); promoted exact-name coverage remains `full` / `authoritative`. Observed nuance: the wire emits `ExpiryUpdated` alone for a direct renew but the adapter derives both `ExpiryChanged` and `RegistrationRenewed` from that one log with registry-family provenance | covered_e2e(renewal_preserves_promoted_coverage_and_registry_edges_follow) |
| Registry token transferred (`TransferSingle` sale and two-item `TransferBatch`) | each positive-value token item derives one `TokenControlTransferred` directly from the canonical ERC-1155 log, with distinct resource and event identity for each batch item (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L194 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L201 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L208 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/erc1155/ERC1155Singleton.sol:L210 @ ens_v2@48b3e2d). The direct transfer fact moves exact-name registrant plus address-name registrant and token-holder relations to the buyer and removes the seller. Any paired `EACRolesChanged` revoke/grant remains separate `PermissionChanged` evidence; callbacks are suppressed, so the transfer emits no `TokenRegenerated` (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L484 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L494 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/access-control/EnhancedAccessControl.sol:L226 @ ens_v2@48b3e2d) | covered_e2e(reserved_labels_foreign_registrar_and_token_sale) |
| Resolver set, changed, and zeroed at the watched registry | registration emits `ResolverUpdated` only for a nonzero resolver (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L474 @ ens_v2@48b3e2d); the edge follows set/change/zero with the zero-set deriving a NULL-resolver `ResolverChanged` (proper detach) and `name_current` clearing the facet. Persistent live polling proves registry state survives separate registration, resolver, subregistry, token-regeneration, and unregister polls without false expiry history (`ens_v2_registry_state_survives_distinct_live_polls`); unregister also emits explicit null `ResolverChanged` and `SubregistryChanged` terminal boundaries for the attached roles. That scenario generation-covers its registry target during automatic startup, so it does not claim that a first-ever registry discovered at the live tip can proceed without the documented bounded catch-up. The composed three-resolver-change plus subregistry attach/detach row runs through standard automatic intake and asserts the parent registry's edge sequence, the cleared current resolver facet, and exactly one registry-scoped `PermissionChanged` for the registered resource and owner from `EACRolesChanged`; it does not assert historical coverage for the discovered target contracts. The registrar defines the registration role bitmap and calls the registry (upstream: .refs/ens_v2/contracts/src/registrar/ETHRegistrar.sol:L17 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/registrar/ETHRegistrar.sol:L151 @ ens_v2@48b3e2d), the registry's nonzero-owner path grants it on the constructed resource (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L463 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/registry/PermissionedRegistry.sol:L469 @ ens_v2@48b3e2d), and the grant compares the existing bitmap before emitting the change (upstream: .refs/ens_v2/contracts/src/access-control/EnhancedAccessControl.sol:L267 @ ens_v2@48b3e2d) (upstream: .refs/ens_v2/contracts/src/access-control/EnhancedAccessControl.sol:L274 @ ens_v2@48b3e2d) | covered_e2e(resolver_and_subregistry_edges_follow_set_change_zero) |
| Record write and clearRecords on a discovered v2 resolver | automatic ENSv2 bootstrap fetches the finite-known-start resolver history in the same startup invocation; text and address writes derive `RecordChanged`, clearRecords derives `RecordVersionChanged` (upstream: .refs/ens_v2/contracts/src/resolver/PermissionedResolver.sol:L253 @ ens_v2@ccaeb58b) (upstream: .refs/ens_v2/contracts/src/resolver/PermissionedResolver.sol:L254 @ ens_v2@ccaeb58b), and the public exact-name route reports its record inventory as `unsupported` because that summary is not yet projected for ENSv2 | blocked(the B2 coverage-recovery cut removed the same-startup discovered-resolver history fetch and its scenario) |
...
| ERC1155 operator approval surface | — | blocked(direct `TransferSingle` and `TransferBatch` ownership facts are admitted and covered above; `ApprovalForAll` remains unadmitted because no current ownership, permission, or consumer projection contract uses operator approvals) |
| UUPS implementation rotation on discovered registries/resolvers | `Upgraded` derives normalized history on the emitting contract without changing its identity, parent link, or resolver link. UserRegistry is UUPS-upgradeable (upstream: .refs/ens_v2/contracts/src/registry/UserRegistry.sol:L20 @ ens_v2@ccaeb58); the event is declared by ERC-1967 (upstream: .refs/basenames/lib/openzeppelin-contracts/contracts/interfaces/IERC1967.sol:L13 @ basenames@1809bbc). | covered_normalized_only(declared proxy: discovery_semantics::declared_proxy_upgraded_history_from_prefetched_raw_facts; RegistryCreated emitter: ens_v2_registry_created_scope_admits_only_same_position_and_later_proxy_history; ResolverUpdated target: discovered_resolver_upgraded_history_respects_activation_block) |
...
The primary-name rows below use ENSv1's Base L2ReverseRegistrar
...
| Declared ERC-1967 proxy emits `Upgraded` | the proxy constructor's prefetched `Upgraded` log derives one normalized history event on the emitting controller proxy with the installed implementation address (upstream: .refs/basenames/lib/openzeppelin-contracts/contracts/interfaces/IERC1967.sol:L13 @ basenames@1809bbc) (upstream: .refs/basenames/lib/openzeppelin-contracts/contracts/proxy/ERC1967/ERC1967Proxy.sol:L27 @ basenames@1809bbc) | covered_normalized_only(discovery_semantics::declared_proxy_upgraded_history_from_prefetched_raw_facts) |
...
| Resolver rotated to a second (unadmitted-profile) instance, then to zero | binding follows registry `NewResolver`; match-all Base resolver intake retains the prefetched raw `TextChanged` log without a resolver discovery edge, and address-only code-hash observations prove the target differs from the manifest seed. That persisted mismatch is the record-consumption gate, so zero `RecordChanged` events derive for the name; the zero rotation clears the declared resolver facet | covered_backfill_only(unadmitted_resolver_rotation_stays_profile_gated_then_clears) |
| Legacy ReverseRegistrar claim/setName (registry-driven reverse hierarchy) | claims write the registry under `80002105.reverse` via `setSubnodeRecord` (upstream: .refs/basenames/src/L2/ReverseRegistrar.sol:L158 @ basenames@1809bbc). A claim-only ingest derives `NewOwner` as `SubregistryChanged`, and a combined claim-plus-setName replay retains both canonical child-assignment history events. `NewResolver` for the unknown reverse node creates neither resolver discovery history nor a resolver edge; the reverse-node `NameChanged` survives only as a raw fact, and without an admitted primary-claim source or known reverse parent no normalized `RecordChanged` or `children_current` reverse placeholder appears and the primary candidate stays absent | covered_e2e(legacy_reverse_registrar_stays_registry_and_raw_record_only) |
...
| Contenthash (and other unadmitted-family) writes on the admitted L2Resolver | the pinned L2Resolver composes `ContentHashResolver` (upstream: .refs/basenames/src/L2/L2Resolver.sol:L29 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/L2Resolver.sol:L33 @ basenames@1809bbc), whose setter emits `ContenthashChanged` (upstream: .refs/basenames/src/L2/resolver/ContentHashResolver.sol:L32 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/resolver/ContentHashResolver.sol:L34 @ basenames@1809bbc); the raw contenthash log is retained at the watched instance, the profile gate rejects it before derivation, zero normalized events derive, and the inventory reports the explicit `not_observed_on_current_resolver` gap; the admission narrowing is documented in docs/upstream.md § Known divergences | covered_e2e(l2_resolver_records_clear_and_contenthash_gap) |
| setTTL / setRecord → NewTTL decode | — | blocked(same manifest/adapter drift as the ENSv1 TTL row and the same defect: `basenames_base_registry/v2.toml` declares the `NewTTL` fragment, but the shared unwrapped-authority adapter has no NewTTL observation path (`crates/adapters/src/ens_v1_unwrapped_authority/observation.rs` proceeds from NewOwner/Transfer/NewResolver straight to resolver events) — doc-first or adapter change required before an accurate row) |
...
The shipped `mainnet` profile spans ENSv1 on `ethereum`, Basenames on
`base-mainnet`, two active Basenames source families that run on the
Ethereum chain (`basenames_l1_compat` v1 and `basenames_execution` v2) —
"L1 Basenames families" below — and the separate
shadow `ens_execution` family. The protocol-specific sections above use
single-protocol, single-chain corpora. The scenarios below close the intake
composition gap with the eleven non-`ens_execution` families across both
chains and the L1 Basenames families. They do not mirror the shadow `ens_execution`
family, do not claim full-mainnet-profile coverage, and do not cover the
blocked CCIP transport path.
...
| Scenario | Key assertions | Status |
| --- | --- | --- |
| Composed-corpus equivalence | one corpus, both anvils, and an eleven-family non-`ens_execution` mirror (five ENSv1 intake families + four Basenames base-mainnet families + the two L1 Basenames families): per-chain canonical checkpoints coexist, and each protocol's exact-name body equals its single-protocol baseline after normalizing corpus-minted identifiers — including the found nuance that `authority_key`'s third segment is the per-corpus contract-instance ordinal (9 vs 1 across corpora), so the key is corpus-relative rather than purely chain-derived. Shadow `ens_execution` is outside this row and covered separately by the direct verified-resolution slice | covered_e2e(composed_mainnet_profile_serves_both_protocols_without_leakage) |
...
| Cross-chain perturbations | a LIVE mid-session reorg on the Base chain converges Base to the winning branch (losing rows orphaned by block hash) while the ethereum chain keeps zero orphaned rows, an unmoved canonical checkpoint, and `GET /v1/names/ens/steady.eth` still serves the active name after projection replay | covered_e2e(base_reorg_leaves_ethereum_canonicality_untouched) |
| L1-compat declared side | both L1 Basenames families (`basenames_l1_compat` v1, `basenames_execution` v1+v2) sync their admission into the composed corpus as stored manifest state on `ethereum-mainnet` and are served by `GET /v1/manifests/basenames` (live runs derive no manifest bookkeeping events — those are backfill-only extras per the perturbation-corpus parity rule), while the undeployed placeholder role stays silent; the CCIP transport hop stays blocked | covered_e2e(composed_mainnet_profile_serves_both_protocols_without_leakage) |
...
| Merged non-`ens_execution` mainnet-family mirror + dual-anvil pipeline run | 9 | covered: `manifests::generate_local_mainnet_composed_profile` unions eleven families (five ENSv1 intake, four Base, two L1 Basenames) into one root, while intentionally omitting the twelfth checked-in family, shadow `ens_execution`; `pipeline::indexer_run_until_chain_checkpoints` waits each chain's canonical checkpoint under one live session |

## Phasing

The original order put the determinism harness before breadth so later
scenarios could reuse it. The shipped perturbation tests (phase 3) apply that
harness to one representative rich corpus; reuse across the full matrix
remains future work.
...
| 7 | ENSv2 declared-state matrix | done as a classified pass — fresh registration and live expiry/re-registration remain covered; B2 removed the driver-backed discovered-source scenarios, and their resulting coverage gaps are explicit in the ledger |
...
| 9 | Cross-protocol composition: eleven non-`ens_execution` mainnet families, dual-anvil corpus, composition rows | done — all six rows covered over that eleven-family corpus, including a live one-chain reorg; shadow `ens_execution` remains a separate verified-resolution slice, `authority_key` is corpus-relative (instance ordinal), and manifest bookkeeping events are backfill-only |
