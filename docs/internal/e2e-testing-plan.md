...
| Unadmitted custom resolver emits records | — | blocked(the former scenario deployed a byte-identical PublicResolver copy, which is correctly admitted by code hash and is now covered by `byte_identical_public_resolver_copy_converges_to_admitted_profile`; no current e2e scenario deploys a materially different ENSv1 resolver while exercising generic retained resolver events, so pending-profile gating is covered only by adapter, projection, and API integration tests) |
...
| Claim routed through an unadmitted resolver | a live claim whose generic `NameChanged` is retained as one unanchored normalized observation, but without an admitted supported name profile it carries no `primary_claim_source` and publishes no declared candidate — the reverse-plane analog of the pending-profile resolver row | covered_e2e(unadmitted_reverse_resolver_keeps_candidate_absent) |
...
| Role bitmap grant/revoke, root vs resource scope | effective powers per registry vocabulary; revoke removes current subject row | blocked(the deleted ENSv2 matrix was the only combined root/resource grant-revoke e2e; root projection coverage remains in `root_apex_attach_and_root_scope_roles`, and resource admin rendering remains in `reserved_labels_foreign_registrar_and_token_sale`) |
| Subregistry attached, then swapped | SubregistryChanged derives for attach and swap at the parent registry, the parent resource survives the swap, automatic ENSv2 bootstrap fetches each finite-known-start discovered child registry in the same startup invocation, the new child's registration projects into the parent's children, and the replaced child's name leaves the current listing | blocked(the B2 coverage-recovery cut removed the same-startup discovered-child fetch and its matrix scenario; attach/detach coverage remains in `resolver_and_subregistry_edges_follow_set_change_zero`) |
...
| Unregister → re-register | resource and token lineage both advance across the cycle; live intake reaches the post-cycle checkpoint, keeps the stable name surface, closes the released resource binding before publishing the successor, and exact-name readback serves the new owner/resource | blocked(the deleted ENSv2 matrix was the only single scenario for this exact transition; unregister remains normalized-only in `ens_v2_registry_state_survives_distinct_live_polls`, while expiry/grace re-registration remains covered by `expiry_passes_then_reregistration_advances_lineage`) |
...
| Resolver rotated to a second (unadmitted-profile) instance, then to zero | binding follows registry `NewResolver`; an initial backfill discovers the resolver and a focused watched-target backfill retains its raw `TextChanged` log and code hash, so the persisted mismatch with the manifest seed's code hash is the actual record-consumption gate and zero `RecordChanged` events derive for the name; the zero rotation clears the declared resolver facet. **REVIEW POINT**: live intake hangs on this rotation-to-discovered-instance chain (the Base sibling of the reproduced live-intake hang family — the ENSv1 twin ingests live cleanly), so the scenario remains backfill-only and the stored inventory remains a stub — the wire-level unadmitted-binding inventory rendering stays untestable on Base pending the intake fix (the ENSv1 twin pins that shape) | covered_backfill_only(unadmitted_resolver_rotation_stays_profile_gated_then_clears) |
