...
| Label preimage revealed later | placeholder upgrades to the real name | planned(8) |
| Set TTL on a node (registry, and routed via the wrapper) | `NewTTL` → `AuthorityEpochChanged` with a TTL payload — an admitted derivation never yet produced from a real log in any scenario; no owner/resolver side effects | planned(8) |
| Registry-only name tree under a non-.eth TLD | owner, resolver, and records derive for a 2LD with no registrar family at any ancestor (the shape of DNSSEC-claimed trees); exact-name coverage and registration facets pinned for the no-registration-anchor shape | planned(8) |
...
| Write and delete records across the remaining admitted families (ABI, interface, DNS RRset, zonehash, forward name()) via multicall | the inventory claim is demonstrated for 4 of ~13 admitted families; keyed-family shapes (contentType, interfaceID) never produced; `DNSRecordDeleted` (upstream: .refs/ens_v1/contracts/resolvers/profiles/DNSResolver.sol:L186 @ ens_v1@91c966f) is the only admitted deletion event — supersession-by-delete pinned; forward name() as a forward record rather than a reverse-claim carrier | planned(8) |
...
| Claim whose forward resolution mismatches | claimed present, verified reports mismatch (needs execution plane) | planned(8) |
...
| Claim without a name record (claim/claimWithResolver) | the reverse node's registry owner rotates via setSubnodeRecord with no `NameChanged` (upstream: .refs/ens_v1/contracts/reverseRegistrar/ReverseRegistrar.sol:L84 @ ens_v1@91c966f) — no candidate minted; reverse-node `NewOwner` never mistaken for a claim | planned(8) |
| Claim set for another address by an authorised third party | claimForAddr/setNameForAddr (upstream: .refs/ens_v1/contracts/reverseRegistrar/ReverseRegistrar.sol:L123 @ ens_v1@91c966f) — candidate derives from the node labelhash, not the tx sender | planned(8) |
| Claim routed through an unadmitted resolver | a live claim whose `NameChanged` never derives: declared candidate honestly absent — the reverse-plane analog of the unadmitted-resolver row | planned(8) |
