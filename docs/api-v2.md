...
| `permission_resource_id` | the handle that selects an address-name row's permission rows on the permissions route's `registration_id` filter: the name's `registration_id` while it serves a supported current registration (for a wrapped `.eth` name the BaseRegistrar lease, although the rows live on the NameWrapper resource), otherwise the permission authority resource UUID itself | new in v2 |
