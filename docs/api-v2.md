...
| `record_resource` | on `GET /v1/permissions` rows and resolver `/roles` rows for a grant on an ENSv2 record-ID resolver: the setter argument the granted resource stands for, decoded — `{kind: "address", hash, coin_type}`, `{kind: "text" \| "data", hash, key}`, `{kind: "abi", hash, content_type}`, `{kind: "interface", hash, interface_id}`, or `{kind: "argument", hash, selectors}` when one argument authorizes several setters | permission-event `selector`, `scope_detail.resource_selector` |
