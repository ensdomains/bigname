...
guards, runs the live manifest-drift audit, inspects the runtime watch plan, and
checks the API process readiness endpoint. It does not deploy, contact external
...
same release-blocking meaning as a local non-zero exit, except that missing
