...
> history until the B4 verification and Stage C cutover runbook replaces them; use
...
semantic repair decision. Each compatible update transaction locks and advances
the full-replay input revision before mutating the surface metadata, so a worker
cannot resume or publish a durable projection stage built from the earlier
metadata; completion markers and an automatic replay attempt from that revision
are invalidated in the same transaction.
