...
- `phase7-verified-primary-provenance-contract-clarification`: the shared contract docs now freeze `verified_primary_name.provenance` as a strict verification-local `Provenance` refinement for the exact requested tuple under the same top-level `provenance.execution_trace_id`; when later published, it may narrow only the persisted verification trace context (`normalized_event_ids`, `raw_fact_refs`, `manifest_versions`, `derivation_kind`) and must not publish `verified_primary_name_lookup` / `verified_primary_name_invalidation` hook material, restate claimed-row provenance, widen request-key or invalidation identity, or imply primary-name coverage graduation; public field publication plus projection/API/OpenAPI wiring remain pending
- `phase7-ens-verified-primary-route-readback-bootstrap`: the shipped `GET /v1/primary-names/{address}` route now reads back persisted ENS `verified_primary_name` results for the exact requested tuple in `mode=verified|both`, using the already frozen mixed-route envelope and the same machine-readable publication in `docs/api-v1.openapi.json`; declared claim behavior remains bootstrap exact-tuple readback under the separately frozen `status` and `raw_claim_name` boundaries, tuple-present fallback `unsupported` behavior still applies when no persisted ENS readback exists, and route-level primary-name coverage stays `unsupported`
