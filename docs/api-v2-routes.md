...
| Reverse-claim name records | Served outside the grammar | No record key. Reverse and primary-name routes serve name records that carry a reverse-claim attribution; the primary-name projection consumes only those rows. ENS defines the resolver name field for reverse records. (upstream: .refs/ens_v1/contracts/resolvers/profiles/INameResolver.sol:L4-L11 @ ens_v1@91c966f) |
| General resolver name records | Outside the grammar | No public key and no serving surface. A `NameChanged` on an ordinary (non-reverse) resolver is retained as normalized record history, but the record routes reject the `name` family and the primary-name projection ignores rows without a reverse-claim attribution. (upstream: .refs/ens_v1/contracts/resolvers/profiles/INameResolver.sol:L4-L11 @ ens_v1@91c966f) |
...
| ENSv1 arbitrary data records | Outside the grammar | No public key. ENS defines string-keyed arbitrary byte data, and its public resolver includes that profile. (upstream: .refs/ens_v1/contracts/resolvers/profiles/IDataResolver.sol:L5-L21 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/resolvers/PublicResolver.sol:L20-L30 @ ens_v1@91c966f) |
