...
`ens_v1_registrar_l1` owns `.eth` BaseRegistrar at `start_block = 9380410`[^subgraph-l122] plus the legacy, wrapped, and current ETHRegistrarController contracts as label-bearing intake (LegacyEthRegistrarController `9380471`,[^subgraph-l145] WrappedETHRegistrarController `16925618`,[^v1-wrapethrc-l640] current ETHRegistrarController `22764821`).[^v1-ethrc-l706] Controllers do not split into a separate source-family owner. The admitted controller set is the ordinary source of `.eth` registration and expiry facts, because controller-sourced events are what carry the human-readable label: admission is what binds a registration to a name rather than a token id. The BaseRegistrar's own `NameRegistered` and `NameRenewed` — the events the registrar declares (upstream: .refs/ens_v1/contracts/ethregistrar/IBaseRegistrar.sol:L15 @ ens_v1@91c966f) when it writes `expiries[]` (upstream: .refs/ens_v1/contracts/ethregistrar/BaseRegistrarImplementation.sol:L142 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/BaseRegistrarImplementation.sol:L166 @ ens_v1@91c966f) — are the fallback: a registrar event whose transaction carries no admitted controller event for the same label derives the fact itself, at its own position, so later logs in the transaction see it. The fact records the registrar payload's owner, which is the token owner. A manifest authorizes the fallback by declaring `RegistrationGranted` on the registrar's own `NameRegistered`; the Sepolia profile declares only its migration output and so keeps its numeric events out of it. A renewal updates the stored expiry; a registration establishes a registrar identity with no label (`surface_known = false`); both carry `controller_admitted = false` in their after-state. Within a transaction the registrar's log precedes the controller's (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L288 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L333 @ ens_v1@91c966f), so the pairing is decidable once the block is read, and an admitted controller's output is unchanged by the fallback. A rotation is therefore loud rather than silent: renewals through a controller this manifest does not admit keep the expiry current instead of settling a spurious `RegistrationReleased`, and registrations through it are served as flagged labelhash-only registrations until the controller is admitted here, which remains a manifest change. The full divergence is recorded in [`upstream.md`](upstream.md#known-divergences). The wrapped controller's `NameRenewed` additionally derives a wrapper-resource expiry observation under this family because its renewal calls `NameWrapper.renew`, which stores registrar expiry plus grace without emitting `ExpiryExtended`. (upstream: .refs/ens_v1/deployments/mainnet/WrappedETHRegistrarController.json:L656 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L318 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L333 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/wrapper/NameWrapper.sol:L337 @ ens_v1@91c966f)
