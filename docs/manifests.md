...
Registrar-controller coverage remains a plaintext-enrichment asymmetry against the mainnet deployment profile; ordinary registrar existence and expiry no longer depend on that coverage, and resolver-log coverage for the approved four-address set is no longer asymmetric.
