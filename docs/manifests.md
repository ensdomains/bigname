...
ENSv1 dynamic resolver indexing is required for consumer-usable declared record coverage. The mainnet PublicResolver manifest entry is a bootstrap seed, not the complete ENSv1 resolver universe. A follow-on manifest version must add a resolver discovery rule from the admitted ENSv1 registry source to `ens_v1_resolver_l1`: each canonical nonzero `NewResolver(node, resolver)` observation from an admitted registry emitter creates or refreshes the node-to-resolver binding and admits the resolver address as a resolver contract instance through a discovery edge; a zero-address resolver observation closes only that node-to-resolver binding. The upstream registry exposes `NewResolver(bytes32 indexed node, address resolver)`, emits it from `setResolver`, and also emits it when `setRecord` / `setSubnodeRecord` change resolver state through `_setResolverAndTTL` (upstream: .refs/ens_v1/contracts/registry/ENS.sol:L12 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L89 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/registry/ENSRegistry.sol:L174 @ ens_v1@91c966f). Resolver discovery admission is not resolver-profile support: a registry-selected arbitrary address may be watched and represented as a `contract_instance_id`, but typed record, record-version, and resolver-local authorization facts may be appended only after a supported resolver profile is admitted for that instance and record family through an explicit mechanism such as code-hash / implementation allow-listing, ERC165 interface probing, ABI-family admission, or supported resolver-event observation. The static PublicResolver support profile is the current cited profile seed (upstream: .refs/ens_v1/contracts/resolvers/PublicResolver.sol:L20 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/resolvers/PublicResolver.sol:L131 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/resolvers/ResolverBase.sol:L17 @ ens_v1@91c966f). Until that follow-on lands, ENSv1 record inventory, record cache, and resolver overview must not claim consumer replacement for names whose current resolver is outside the statically admitted PublicResolver or lacks supported profile admission.
...
Basenames dynamic resolver indexing follows the same manifest/discovery rule on Base Mainnet. `basenames_base_resolver` starts with the upstream default `L2Resolver`, but it must not be treated as the only possible Base-side resolver for consumer replacement. A follow-on manifest version must add a resolver discovery rule from the admitted `basenames_base_registry` root to `basenames_base_resolver`: each canonical nonzero registry `NewResolver(node, resolver)` observation creates or refreshes the node-to-resolver binding and admits the resolver address as a Base resolver contract instance through a discovery edge; a zero-address resolver observation closes only that node-to-resolver binding. Upstream Basenames registry stores resolver addresses per node, emits `NewResolver` from `setResolver`, and emits it when `setRecord` / `setSubnodeRecord` change resolver state through `_setResolverAndTTL` (upstream: .refs/basenames/src/L2/Registry.sol:L19 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/Registry.sol:L132 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/Registry.sol:L223 @ basenames@1809bbc). Resolver discovery admission is not resolver-profile support: a registry-selected arbitrary address may be watched and represented as a `contract_instance_id`, but typed record, record-version, and resolver-local authorization facts may be appended only after a supported resolver profile is admitted for that instance and record family through an explicit mechanism such as code-hash / implementation allow-listing, ERC165 interface probing, ABI-family admission, or supported resolver-event observation. The static `L2Resolver` support profile is the current cited profile seed (upstream: .refs/basenames/src/L2/L2Resolver.sol:L22 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/L2Resolver.sol:L209 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/resolver/ResolverBase.sol:L30 @ basenames@1809bbc). This Base-side discovery rule does not discover or widen the separate Ethereum Mainnet L1 resolver used by `basenames_l1_compat` / `basenames_execution`, and it does not admit offchain gateways. Until it lands, Basenames record inventory, record cache, and resolver overview must not claim consumer replacement for names whose current Base-side resolver is outside the statically admitted `L2Resolver` or lacks supported profile admission.
