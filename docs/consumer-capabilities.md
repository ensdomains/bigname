...
| Resolver overview | `GET /v1/resolvers/{chain_id}/{address}` | Resolver metadata, total section counts with deterministic samples capped at 100 items, and separately paginated complete alias, record-link (ENSv2 record-ID resolvers: which nodes share which record, the default record included), per-registration role, and record-shaped bound-name collections, including [expiry-effective](glossary.md#expiry-effective-namewrapper-fuse-word) ENSv1 NameWrapper metadata when backed. |
