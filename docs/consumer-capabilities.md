...
| role holders | `GET /v1/resources/{resource_id}/permissions` | One current row per `(resource_id, subject, scope)`, plus projection-owned per-resource support metadata even when the row set is empty. |
