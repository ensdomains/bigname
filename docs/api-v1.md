...
- `verified_primary_name` is frozen to the shipped mixed-route field boundary `{status, name?, unsupported_reason?, failure_reason?}`; `name`, when present on persisted ENS readback, uses the shared `NameRef` shape, `raw_claim_name` never appears on this execution-derived object, and public publication of section-local `verified_primary_name.provenance` remains additive rather than part of the already-published `docs/api-v1.openapi.json` route shape
- `verified_primary_name.provenance` invariants are nevertheless frozen now: when later admitted, it reuses `Provenance` as a strict verification-local refinement for the exact requested tuple, `verified_primary_name.provenance.execution_trace_id` must equal top-level `provenance.execution_trace_id`, and any `normalized_event_ids`, `raw_fact_refs`, `manifest_versions`, or `derivation_kind` it carries must narrow that same persisted verification trace rather than widen it
...
- `verified_primary_name.provenance`, when present, is verification-local provenance for that same exact tuple; `verified_primary_name.provenance.execution_trace_id` must equal top-level `provenance.execution_trace_id`, and it must not publish claim-side lookup / invalidation hook material
