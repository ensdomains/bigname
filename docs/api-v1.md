...
- the bootstrap declared `claimed_primary_name` contract stays exact-tuple and claim-local: `status` is always present, `claimed_primary_name.raw_claim_name` is the only admitted claimed-local field beyond bare status, and `claimed_primary_name.name` plus claim-local section provenance remain deferred
...
- in the shipped Phase 7 bootstrap slice, richer ENS tuple-present payloads remain blocked: reverse tuple presence, reverse-only claim precedence, or verification establishing a concrete normalized name target do not by themselves populate `claimed_primary_name.name` or claim-local section provenance, and they do not widen the exact-tuple `status=invalid_name` gate for `claimed_primary_name.raw_claim_name`
...
- the mixed route continues to use the same already-published bootstrap envelope in `docs/api-v1.openapi.json`; this raw-claim-name clarification does not widen query parameters, add snapshot handling, admit fallback claim sources, or change route-level coverage
