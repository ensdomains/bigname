...
- Persisted-readback routes return their documented stale or not-found state when matching output is missing. Supported ENS verified selectors on the resolution and compact records routes instead execute on demand. An unconfigured provider or a JSON-RPC response recognized as unable to serve the selected block returns `409 stale`. For those on-demand verified record calls, expiration of the configured provider connect or total-response deadline or the CCIP-Read gateway client's configured total-response deadline is a persisted in-band selector execution failure; other provider or gateway transport failures return `409 stale` without caching an outcome, so the next read retries. The ENS/60 primary-name fallback uses the same provider-and-gateway transport split: configured timeouts remain persisted in-band execution failures, while DNS, TLS, connection-reset, and other non-timeout transport failures return `409 stale` before persistence.
...
- A verified-resolution provider or CCIP-Read gateway timeout is an execution result: the affected selector returns the existing in-band `execution_failed` class. Primary-name provider and gateway timeouts likewise use that route's existing in-band execution-failure class. They are distinct from the whole-request `408` backstop.
