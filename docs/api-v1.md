...
| `GET /v1/primary-names/{address}` | App primary-name lookup for an address; defaults to ENS coin type 60, with selected-checkpoint-pinned route-local reverse claim and persisted forward verification when the claim tuple is missing. |
...
- Persisted-readback routes return their documented stale or not-found state when matching output is missing. Supported ENS verified selectors on the resolution and compact records routes instead execute on demand. An unconfigured provider or a JSON-RPC response recognized as unable to serve the selected block returns `409 stale`. For those on-demand verified record calls, expiration of the configured provider response deadline is a persisted in-band selector execution failure; a provider connect-phase timeout or other provider transport failure returns `409 stale` without caching an outcome, so the next read retries. Record-route verified reads execute block-pinned and do not follow CCIP-Read, so no gateway leg applies to them. The ENS/60 primary-name fallback — the one route whose verified execution follows CCIP-Read — uses the same transport split with the gateway leg included: configured provider or CCIP-Read gateway response timeouts remain persisted in-band execution failures, while provider or gateway connect-phase timeouts, DNS, TLS, connection resets, and other transport failures return `409 stale` before persistence.
- Every route has a whole-request deadline. Exceeding it returns `408 request_timeout`; `/healthz` bypasses the process-wide concurrency limiter and load shedding, and its two-second `SELECT 1` uses a reserved one-connection pool. The probe therefore remains ready during HTTP-concurrency saturation and request-pool exhaustion, while a small independent health ceiling prevents unbounded work and the whole-request deadline remains a final backstop. A failed or timed-out readiness connection reports the database as unreachable. `/v1/status` retains the process-wide ceiling because its aggregate database query is not a liveness probe.
- Routes that can trigger verified execution may return `429 rate_limited` when the deployment's client token bucket is enabled. IPv4 addresses are individual keys and IPv6 addresses are grouped by `/64`. The limit applies before execution starts and does not turn a provider failure into an HTTP error. A compact records request in `mode=auto` with no requested record selectors is an indexed read and does not enter verified-execution admission.
...
- A verified-resolution provider response timeout is an execution result: the affected selector returns the existing in-band `execution_failed` class. Primary-name provider and CCIP-Read gateway response timeouts likewise use that route's existing in-band execution-failure class. Provider and gateway connect-phase timeouts abort before persistence so a later request retries. These transport deadlines are distinct from the whole-request `408` backstop.
