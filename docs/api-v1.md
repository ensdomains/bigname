...
| `migrated_at` | RFC 3339 block time of the name's latest activated `MigrationApplied` [migration boundary](glossary.md#migration-boundary), the ENSv1→ENSv2 [migration authority transition](glossary.md#migration-authority-transition) kept as history. It records when the name migrated, not why it has its current authority: the current ENSv2 registration decides that as it would for any name. Present only with `authority=ens_v2`, so a migrated name whose ENSv2 registration was later released keeps it (it stays released under ENSv2), while one that a later ENSv2 reservation hands back to ENSv1 omits it, as do a name registered directly in ENSv2 and one on `ens_v1` or `ens_v0` | new in v2 |
