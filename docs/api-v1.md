...
- `claimed_primary_name.name` remains deferred to a later additive claimed slice and appears only for `status=success` if shipped; in the shipped persisted-readback slice, `verified_primary_name.name` appears only for `status=success` or `status=mismatch`, where the route established a concrete normalized name target for verification
- the mixed route continues to use the same already-published bootstrap envelope in `docs/api-v1.openapi.json`; this additive slice changes only the documented/readback contents of `verified_primary_name` for persisted ENS exact-tuple results and does not widen query parameters, snapshot handling, or route-level coverage
