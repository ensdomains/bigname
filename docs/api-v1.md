...
- the shipped declared `claimed_primary_name` readback remains bootstrap status-only for the exact tuple: current public responses use only the status-shaped declared object and do not yet populate `claimed_primary_name.raw_claim_name`, `claimed_primary_name.name`, or claim-local section provenance, even when tuple-local claim inputs are already persisted for later additive readback and verified invalidation
- `claimed_primary_name.raw_claim_name` is the only admitted claimed-local field beyond that bootstrap status shape; if a later additive claimed slice publishes it, it is copied verbatim from `primary_names_current.raw_claim_name` for the exact requested tuple, appears only for `status=invalid_name`, and must not be synthesized from normalized name identity, resolver-backed name data, or verification-derived identity
...
- `primary_names_current(address, coin_type, namespace)` is the only admitted claim-side lookup / invalidation anchor for the current bootstrap declared handling and for the verified tuple; persisted claim-local inputs there do not by themselves publish richer `claimed_primary_name` or `verified_primary_name` fields
...
- the mixed route continues to use the same already-published bootstrap envelope in `docs/api-v1.openapi.json`; this claim-local-fields clarification does not itself publish `claimed_primary_name.raw_claim_name`, widen query parameters, add snapshot handling, or change route-level coverage
...
- when the admitted declared claim cannot be normalized, `claimed_primary_name` may still return `status=invalid_name`; `raw_claim_name` is the only claim-local field pre-admitted for a later additive claimed slice in that case, while `verified_primary_name.status=invalid_name` remains limited to verification-local fields
...
- claim-local provenance inputs may already be stored with the tuple row for lookup / invalidation support, but public `claimed_primary_name.provenance` remains deferred; when later shipped, it is claim-local declared provenance, must strip any `verified_primary_name_lookup` / `verified_primary_name_invalidation` hook material, and must not introduce a second `execution_trace_id`
