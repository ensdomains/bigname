...
- top-level `provenance` is the only response-wide join between claim-side inputs and any persisted verification trace; `claimed_primary_name.provenance` and any later `verified_primary_name.provenance` must remain strict refinements of that top-level identity rather than a second route-level truth system
