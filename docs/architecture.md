...
An ENSv1 or Basenames registry-owner observation for a node without a materialized `NameSurface` is likewise retained only as interpreter state; it does not independently create a public surface, resource, or binding. Once a registrar or wrapper observation materializes the surface, that retained direct-registry authority may become its fallback. If release of the active registrar lease makes a nonzero retained registry owner authoritative, the release boundary must materialize the registry-anchored resource and open its replacement `SurfaceBinding` in the same interpret batch. The resource and binding use the block-boundary provenance because release is derived from the block timestamp rather than a release log. This rule does not widen ENSv2 registry-name topology or emit either side of a parent-child binding for an otherwise unknown registry-only surface.
