...
record does not exist. Interpret persists that handoff even when a same-owner
`Transfer` has no other state delta, and emits linked zero-resolver events when
the handoff retracts an already surfaced old-registry pointer from every
registry, registrar, or wrapper resource to which it was linked. Later
...
synchronization and phase redo path. The one drift that leaves a mark in
served history is an ENSv1 registrar controller the manifest does not admit:
its registrations and renewals surface as `controller_admitted = false`
registrar facts (see [Source families](#source-families)), which is a signal
to query for, not an alert.
