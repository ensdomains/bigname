...
ENSv1 authority moves (wrap, unwrap, re-registration) carry the identity change in `resource_id` and `token_lineage_id`; ordinary lifecycle stays `declared_registry_path`. A new `SurfaceBinding` row appears only when the bound `resource_id` changes — transfer and expiry within the same anchor do not. For a controller registration with a resolver, the controller first asks the registrar to register the name to `address(this)`, then writes the final registry owner and resolver before it emits `NameRegistered` (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L294 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L296 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L301 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L303 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/ethregistrar/ETHRegistrarController.sol:L333 @ ens_v1@91c966f). Basenames instead writes the final owner directly before its registrar emits `NameRegistered`, so this path has no transient controller-owner epoch (upstream: .refs/basenames/src/L2/BaseRegistrar.sol:L423 @ basenames@1809bbc) (upstream: .refs/basenames/src/L2/BaseRegistrar.sol:L425 @ basenames@1809bbc). When block-scoped replay has preloaded an older registry-only authority for that name, same-transaction `RegistryOwnerChanged` and `ResolverChanged` observations that establish the incoming registration are therefore deferred and attributed to the new registrar resource. Permission changes that close the preceding registry-only authority stay on that preceding resource in either flow so they supersede its grants; the registry-only resource row remains in the batch while that history references it. A transient registration-controller owner observation and that controller's matching self-grant and self-revoke are setup artifacts rather than a separate authority transition. Record writes, renewals, and wrapper observations retain their event-time authority. This registration-setup rule does not define a registry-only `RegistrationGranted` pre-state contract.
