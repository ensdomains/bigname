...
Registry-name suffix text is ENS-normalized for name surfaces and projection joins. Raw label text keys the live topology maps, so the current parent pointer and child claim must still agree on the same raw label. When a registration's label normalizes to a name that already has a current binding, it retires that older binding so one normalized name maps to one current binding. This collision rule is recorded in [`upstream.md` § Known divergences](upstream.md#known-divergences).
