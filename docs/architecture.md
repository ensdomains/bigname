...
Support for a name's exact-name read follows its authority decision and nothing
else. When Project selects the ENSv2 arm for a name and that selection carries no
...
(upstream: .refs/ens_v2_sepolia_20260916/contracts/src/registrar/ETHRegistrar.sol:L151-L171 @ ens_v2_sepolia_20260916@366de741),
...
ownership change follows it, the name is a
...
An existing ENSv1→ENSv2 migration proof retains precedence.
...
for both configured Mainnet and Sepolia ENS deployment profiles. Before the
...
and a live ENSv1 name plus a current ENSv2 registration selects ENSv2. Neither
choice compares the ages of the two arms' facts.
An ENSv1 lease that stays live under a name ENSv2 holds keeps the visibility it
has after a proven migration: its events stay in name history and its
permission rows stay in the permission views, but it supplies no current field
of the name.
...
persistent disagreement remains a data-integrity failure. The retried class is
every provider read failure that ingest does not already treat as transient, so
it also covers a truncated or otherwise unreadable response: a one-off
truncation recovers within the same window, and one that outlasts both retries
is terminal. Each retry logs a warning with the window, attempt, and cause.
Database writes, Coinbase SQL intake, and local reth reads are outside this
recovery loop.
