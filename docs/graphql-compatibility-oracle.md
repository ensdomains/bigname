...
complete effective-owner scalar family retains the two `owner` and `owner_in`
claims and adds 18 exact `input:` claims. This changes the exact `#670/T3`
remainder from 197 to 179 without changing any enum claim. Its four residual
classes remain wrapper-authority names, zero or masked registry owners, names
without a projected ownership event, and state-derived effective-controller
changes from resource-scoped `PermissionChanged` or owner-less
`AuthorityEpochChanged`; both positive and negative members exclude them.
