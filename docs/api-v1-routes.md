...
- A wrapper-resource permission page may therefore be empty. That empty page means no current permission row was projected; it does not prove the holder has no on-chain wrapper powers or that active fuses were applied to a complete grant set. Wrapper-holder permission replacement remains unsupported until that projection is implemented.
