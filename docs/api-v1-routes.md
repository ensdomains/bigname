...
- A pinned lookup with no resolver, empty name, or wrong namespace returns `claimed_primary_name.status=not_found` with ENS reverse-RPC partial coverage. A nonblank unnormalizable reverse name returns `claimed_primary_name.status=invalid_name` with the raw claim preserved. Missing API RPC configuration, a completed JSON-RPC failure response, a malformed successful response, or expiration of the configured provider response deadline returns `claimed_primary_name.status=execution_failed` with `failure_reason=resolver_call_failed`; it is never reported as an absent claim. A provider connect-phase timeout, DNS failure, TLS failure, connection reset, or other transport failure instead aborts the request with `409 stale` before trace or outcome persistence, so the next read retries.
- For `mode=verified|both`, the ENS/60 on-demand path applies the verified-name gate before forward RPC. A normalizable claim whose untrimmed raw spelling does not byte-equal its normalized form returns `verified_primary_name.status=invalid_name` with `failure_reason=claim_not_normalized` without calling the Universal Resolver; a nonblank claim that cannot be normalized returns `verified_primary_name.status=invalid_name` with `failure_reason=claim_name_not_normalizable`, also without a forward call. Only a successful claim that passes this gate is verified by calling the ENS Universal Resolver proxy for `addr:60` at the same selected block hash as the reverse lookup.[^v1-ur-deploy][^v1-iur-l44][^v1-iur-l52] Matching address returns `verified_primary_name.status=success`; a concrete nonmatching address returns `mismatch`; an empty `addr` result returns `not_found`; a completed JSON-RPC failure, malformed return, or configured provider or CCIP-Read gateway-client response timeout returns persisted `execution_failed`. Provider or gateway connect-phase timeouts and other HTTP transport failures return `409 stale` without persistence.
...
- Materialized verified-primary readback remains fenced by the exact `primary_names_current(address, coin_type, namespace)` claim row. The ENS/60 route-local trace is the bounded missing-tuple exception: persistence and readback require that exact row to remain absent, readback requires the exact currently selected checkpoint, and route-local and materialized traces are never substituted for each other. Missing-row persistence and readback use the same transaction-scoped per-tuple advisory lock as projection writes, and a row that wins that race is reloaded through the ordinary projection and normalization gate before any new RPC call; work on different tuples remains concurrent. This preserves the projected `claim_name_is_normalized=false` short-circuit before readback while allowing a missing-tuple fallback to be durable.
