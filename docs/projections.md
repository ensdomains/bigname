...
- `claim_name_is_normalized` is a projection-only Boolean. It is true only when a successful tuple's untrimmed raw claim byte-equals `normalized_claim_name` under the pinned normalizer. A successful false value preserves the declared normalized name but deterministically gates verified output before forward execution or persisted readback. Because the flag is derived rather than normalizer-version-keyed, changing the pinned normalizer requires a projection replay-version bump and full replay.
