...
`permissions_current_resource_summary` is the projection-owned companion keyed by `resource_id`. It records the authority classification used to decide holder-enumeration support, route coverage, the optional ENSv2 registry-root `resource_id`, and the chain-position/canonicality evidence for the authority inputs. A summary is published even when the resource has zero current permission rows, so an authoritative empty result remains distinguishable from unsupported wrapper-holder enumeration or an unreplayed authority classification. Full target discovery includes canonical zero-event resources whose identity provenance supplies a positive source-family/manifest-version pair, including the ENSv1 binding-authority pair; this keeps full and keyed rebuild results equal. Keyed replacement publishes one resource's permission rows and companion summary atomically. A full rebuild atomically replaces both families, writes `permissions_current_publication` version 2, and advances its `data_revision`. A keyed rebuild advances that revision only when version 2 is already present, so it cannot graduate a missing or old global reader contract. API role and permission routes consume these projections instead of reading adapter-owned identity or worker replay progress. They verify one unchanged revision around all permission reads so rows and summaries from different commits cannot be returned together; an absent or unrecognized summary fails closed as `partial` / `best_effort` with `resource_permission_authority_not_projected` after compatibility has been established.
