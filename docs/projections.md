...
| `GET /v1/resources/{resource_id}/permissions` | `permissions_current` plus its transactionally co-published per-resource support summary; requires the compatible projection-owned publication version |
| `GET /v1/roles`, `GET /v1/names/{namespace}/{name}/roles` | `permissions_current` plus its transactionally co-published per-resource support summary; `name_current` only for name-to-resource lookup; requires the compatible projection-owned publication version before permission decoding |
...
The `verified_primary_name.provenance` invariant is additive to public publication. When admitted on the exact-tuple persisted-readback class, including the route-local ENS/60 producer, it reuses `Provenance` as a verification-local section refinement over execution output: `execution_trace_id` plus `manifest_versions` only. That execution provenance is also the v1 top-level route provenance, while v1 `chain_positions` and v2 snapshot metadata carry the selected checkpoint. Materialized reads with no execution output still omit top-level route provenance.
