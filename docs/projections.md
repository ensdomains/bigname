...
`permissions_current_resource_summary` is the projection-owned companion keyed by `resource_id`. It records the authority classification used to decide holder-enumeration support, route coverage, the optional ENSv2 registry-root `resource_id`, and the chain-position/canonicality evidence for the authority inputs. A summary is published even when the resource has zero current permission rows, so an authoritative empty result remains distinguishable from unsupported wrapper-holder enumeration or an unreplayed authority classification. Keyed replacement and full rebuild publish the permission rows and companion summary atomically. API role and permission routes consume this companion instead of reading adapter-owned `resources` provenance; an absent or unrecognized summary fails closed as `partial` / `best_effort` with `resource_permission_authority_not_projected`.
