...
The exact-tuple persisted-readback class and ENS/60 on-demand reverse RPC fallback are the primary-name coverage support classes. Persisted ENS uses `source_classes_considered=["ens_v1_reverse_l1","ens_execution"]`; Basenames uses `["basenames_base_primary","basenames_execution"]`; ENS/60 fallback uses `["ens_reverse_rpc"]`. Supported classes publish route-level `status=partial`, `exhaustiveness=non_enumerable`, `enumeration_basis=primary_name_lookup`, and `unsupported_reason=null`.[^v1-revreg-deploy][^v1-ur-deploy][^bn-readme-l22][^bn-readme-l33] Out-of-class tuples, fresh verified-primary execution, and broader address or namespace coverage stay explicit `unsupported` with `source_classes_considered=[]`.
