...
| `primary_names_current` | `(address, coin_type, namespace)` | bootstrap declared `claimed_primary_name` readback + optional exact-tuple `invalid_name` `raw_claim_name` + invalidation context | reverse, primary claim, verified primary events |
...
- projection-owned `claimed_primary_name` is limited to the declared subset `success|not_found|unsupported|invalid_name`; richer claimed payload fields remain additive-only except the exact-tuple `invalid_name` `raw_claim_name` allowance
...
- the only admitted public claimed-local field beyond bare status is `raw_claim_name`; when the route publishes it, it is copied verbatim from `primary_names_current.raw_claim_name` for the same exact `(address, coin_type, namespace)` tuple and only when `claim_status=invalid_name`
...
- tuple presence is a bootstrap lookup and invalidation hook only; it does not by itself widen claim precedence, admit fallback sources, graduate route-level coverage, or imply richer tuple-present claimed payload support beyond the exact-tuple `invalid_name` `raw_claim_name` allowance
