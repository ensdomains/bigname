...
For ENSv1 and Basenames, `resolver_current` summarizes a resolver only after that resolver address is manifest-admitted or resolver-discovery-admitted into the relevant resolver source family and admitted as a supported resolver profile.[^v1-ens-l12][^bn-registry-l132] A topology edge observed from registry state alone does not create a supported resolver overview.
...
- effective resolver code-hash or resolver-profile seed/admission change
...
Invalidation is deterministic and key-scoped. `projection_invalidations` is the shared worker queue for affected projection families plus family-local keys (`logical_name_id`, `resource_id`, address, resolver tuple, or primary-name tuple). The queue has a `state` column; claimable work is `pending`. `projection_normalized_event_changes` is the append-only normalized-event input to that queue, populated by normalized-event storage writers for inserts, admitted semantic content updates, and canonicality-state updates. Continuing semantic replacements use `content_update`; canonicality transitions use `canonicality_update`; one database update may append both. Historical one-time repairs that ran before the `content_update` kind was introduced retain their legacy `canonicality_update` label, and consumers treat either kind as invalidation input. `projection_apply_cursors` records the worker's consumed `change_id` watermark for that input.
...
Manifest, execution, and other non-normalized-event producers enqueue directly into `projection_invalidations` under the same generation rule. The storage-owned `surface_bindings` repair trigger is the bounded identity-row exception: it enqueues `name_current` and `address_names_current` keys when repair updates change `active_to` or `canonicality_state`; adapter code does not write the queue directly. Storage also enqueues `children_current` when a verified label preimage or read-safe parent `name_surfaces` row makes retained canonical registry child edges newly rebuildable. The storage-owned normalized-event repair path has bounded stale-key exceptions when a repaired value adds or changes a projection key: Basenames primary-claim source repair enqueues both the stale and repaired `primary_names_current` tuple keys; ENSv1 reverse-name profile enrichment may add a valid `primary_claim_source` to an otherwise identical unanchored `RecordChanged(name)` observation and enqueues that tuple, but it never removes retained claim provenance. An explicitly unsupported resolver profile instead derives a distinct `record-change-unsupported` event identity without a claim source and resolver-profile publication marks the retained claim event `orphaned`; reactivation makes the same durable claim identity readable again and orphans the unsupported observation. Pending or unclassified profile evidence stops resolver-profile publication before normalized events change. ENSv1 registrar renewal and ENSv1 or Basenames Base registry/registrar event-time resource repairs enqueue old and repaired resource keys for affected resource-keyed projections; ENSv1 registry resolver before-state and authority-epoch resolver-boundary repairs enqueue affected `record_inventory_current` keys; and ENSv1 same-transaction registration setup repair enqueues the repaired `name_current` key plus affected `permissions_current` resource keys. A new invalidation for a key increments that row's generation, clears retry metadata, and returns the row to `pending`; an in-flight apply may delete only the generation it claimed, so a newer change cannot be lost by an older rebuild finishing late.

Resolver-profile convergence is a bounded indexer-owned direct producer because a code-hash or seed change can alter support without changing any normalized-event payload. Manifest/discovery changes first enter the same durable input queue: the indexer force-enqueues their target diff before advancing the revision-fenced resolver-authority journal, so a crash cannot publish an authority snapshot whose projection work was never requested. For every affected address convergence enqueues `resolver_current` by `(chain_id, resolver_address)`. Before absence-aware adapter repair, it snapshots every readable current or historical logical-name binding to that address and all readable retained resources of those names; readable here means `canonical`, `safe`, or `finalized`, never `observed` or `orphaned`. It then enqueues `record_inventory_current` for that pre-repair resource set, including keys made stale when the adapter subsequently orphans an event. These invalidations are written only after resolver-local normalized events have converged and before the corresponding `resolver_profile_input_changes` generation is acknowledged. The indexer does not write either projection table directly.

Resolver-profile adapter repair accepts only the never-destructively-rotated generation-zero raw-log staging corpus. The upgrade migration marks pre-existing corpora as generation one with unknown completeness, so it neither seeds historical resolver repair nor treats the first authority-journal snapshot as a change; only later code-hash or authority changes become pending work. After any deletion or truncation rotates a corpus, repair fails before publishing invalidations or acknowledging an actual input change; surviving minimum and maximum log positions are not absence authority. There is no in-place resolver-profile backfill proof or versioned adapter-snapshot import in the current release. A full database rebootstrap into a new generation-zero corpus is currently required before queued resolver-profile work can converge.
