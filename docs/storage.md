...
instead of recomputing selector plans from persisted identities.

Stored-lineage promotion locks the chain's `discovery_admission_epochs` row
in shared mode before it verifies the watched set and holds that lock through
checkpoint advancement. Manifest sync and discovery admission bump the same
row in the transaction that changes the watched set, so such a change either
commits before verification starts or waits until the verified checkpoint
write commits. It cannot commit between coverage verification and checkpoint
advancement. When that epoch changes, or the process-local coverage frontier is
otherwise empty or reset, verification starts no later than the earliest
explicit watched `active_from_block` through the stored anchor. Closed
historical discovery intervals participate in that lower bound, so a
...
After the widened interval is verified, the process-local frontier memoizes its
lower bound and normal promotion polls extend it instead of rescanning full
history.
...
deactivated. This keeps immutable job history auditable while allowing one or
more complete reruns on the current manifest to restore promotion.
...
The admitted interval repair is losing-branch surface-binding closure repair. Its proof is an orphaned successor binding whose start equals the stored close, or an orphaned ENSv2 `RegistrationReleased` event for the same logical name and predecessor resource. It changes only that readable predecessor's `active_to`: the repaired value is the earliest readable same-chain successor start strictly after the predecessor start, or `NULL` when none exists. The successor boundary is the non-overlap guard. The storage-owned surface-binding update trigger enqueues `name_current` and `address_names_current` invalidations for the changed interval.
