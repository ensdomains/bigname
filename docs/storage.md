...
| `resolver_profile_reconciliation_invalidation_keys` | indexer resolver-profile convergence | crash-safe chain-keyed projection keys streamed from the exact staged targets before adapter publication, then published and removed in bounded pages only after the matching chain-context reconciliation is durable |
