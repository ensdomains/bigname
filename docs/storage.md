...
2026-07-03 Basenames Base registry-only derivation-change repair record: commit 12bcea0 intentionally moved registry-only ENSv1 authority resource derivation from a labelhash-scoped key to the current node/namehash-scoped key. Its checked-in repair record admitted only the Basenames `AuthorityTransferred` parity case; widening beyond that was not anticipated there. The maintainer-ratified option (i) method is the sanctioned Rust repair machinery, not an ad-hoc SQL migration. Observation rows keep the in-place repair path because their event identities do not include the changed authority key: `PermissionChanged` 1,440,626; `AuthorityTransferred` 6,925; and the observation-conflicting portion of `ResolverChanged` is 162,550 rows (`780,482 - 617,932`). Boundary rows use canonicality supersession because their identities include the changed authority key: `basenames_base_registry` `AuthorityEpochChanged` 623,763; `SurfaceBound` 623,762; `SurfaceUnbound` 41,284; and the boundary `ResolverChanged` population bucket 617,932 where `transaction_hash IS NULL`, `log_index IS NULL`, and `after_state.source_event = AuthorityEpochChanged`. The disclosed second widening under the same option (i) ratification adds the `basenames_base_registrar` `AuthorityEpochChanged` before-key subclass: 3,750,998 canonical rows with `before_state.authority_kind = 'registry_only'`, no transaction/log index, and blocks 18,715,358..46,945,560. The reviewed supersession row-count scope for `basenames_base_registry` `AuthorityEpochChanged`/`SurfaceBound`/`SurfaceUnbound` plus the registrar-family `AuthorityEpochChanged` subclass is 5,039,807 rows; the boundary `ResolverChanged` bucket is a separately archived population bucket, not part of the observation-conflict subtraction. The checked-in repository does not contain the deployment corpus needed to recompute these counts.
