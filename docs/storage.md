...
Normalized events are adapter-owned semantic transition rows, not guaranteed-stateless decorations on individual raw logs. Some producers can derive every emitted row from the selected raw fact alone; those stateless replay lanes may be replayed from a block-hash selection. One adapter may still own contextual discovery effects while exposing a normalized-event-only stateless lane, so the central replay contract classifies both the adapter's ordinary dependency model and any narrower stateless lane. Stateful adapters derive `before_state`, resource continuity, authority metadata, resolver state, wrapper state, registrar expiry, and permission provenance from the chronological adapter history. For those adapters, replay that emits or compares transition rows must start from a valid closure boundary and carry adapter state across every physical page in the replay.
...
Current raw-fact normalized replay allows restricted block-hash/source-scoped replay only for adapters classified `stateless_raw_fact`. `stateful_closure_required` and `contextual_dependency_required` adapters fail closed for restricted replay unless the requested range starts at the retained closure boundary and the adapter has an implemented closure/dependency replay session. `--stateless-only` uses the stateless lane in this same central contract; it does not maintain a second adapter allowlist. The central code contract is mirrored here:

| Adapter / producer | Ordinary model | `--stateless-only` lane | Raw-fact restricted replay | Reason and proof |
| --- | --- | --- | --- | --- |
...
| `ens_v1_subregistry_discovery` | `contextual_dependency_required` | Normalized events only | Restricted replay denied; full retained closure replay allowed | Ordinary replay includes contextual discovery reconciliation. The stateless-only lane reads the already-stable contract-instance context for selected registry logs, emits only its normalized rows, and does not insert, deactivate, or reconcile discovery edges. |
...
The stateless-only lane is an explicit replay authority boundary. It re-derives selected rows from canonical raw facts and current manifest/source context. A missing identity is inserted normally. When the same `event_identity` already carries current content, it is unchanged. When its stored content differs but still addresses the same downstream projection keys, replay retains the `normalized_event_id`, replaces the stored semantic content with the current derivation, and appends a `projection_normalized_event_changes` entry through the same normalized-event change path used by replay repairs; a simultaneous canonicality change is recorded by the existing storage trigger rather than duplicated. Projection workers consume that journal and re-derive affected projections. A mismatch that would change a projection key fails closed because the change journal resolves the retained event ID to its current row and cannot recover the old key; that case requires a separately ratified repair which invalidates both keys. The ordinary family-sync and replay upserts remain fail closed on a mismatched identity, and omitting `--stateless-only` preserves the closure/context-dependent selection guard.

This authority does not delete raw facts, synthesize closure state, run adapter checkpoints, mutate adapter-owned identity rows, or reconcile discovery. Each examined identity is logged with `event_identity`, `derivation_kind`, `identity_outcome=inserted|unchanged|superseded`, and `differing_fields`; completion logs aggregate `identities_examined`, `identities_inserted`, `identities_unchanged`, and `identities_superseded`.
...
Ordinary replay does not delete stale `normalized_events` or replace existing payloads for an already persisted normalized-event identity. Its upsert path inserts absent rows and refreshes canonicality for matching identities; conflicting payloads remain mismatches except for the explicitly documented adapter-repair fields below. The explicit projection-key-preserving `--stateless-only` authority described above is the sanctioned exception. Adapter-owned identity rows may be marked `orphaned` only when those rows have no backing normalized event, were produced by the same adapter boundary, and would otherwise overlap the incoming identity interval.

Replay does not mutate `chain_*`, `raw_*`, `backfill_*`, `execution_*`, manifests, discovery rows, public API state, or checkpoint promotion state. It does not write projection output tables directly: normalized-event persistence appends the storage-owned `projection_normalized_event_changes` journal, and explicitly documented key-aware repairs may enqueue `projection_invalidations`; projection workers remain the only owners of projection re-derivation and publication.
