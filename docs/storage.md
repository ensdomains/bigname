...
| `child_registration_events` | Project | Historical membership of each name's [direct child registration](api-v2-routes.md#direct-child-registrations-includechild_registrations) events, rebuildable from canonical interpreted input; name history selects rows through it and reads the events themselves from `normalized_events`. |
