...
Explicit adapter repair is narrower than replay and exists for deterministic adapter bugs where the persisted normalized-event identity is correct but a small payload field was encoded lossily. In minimal raw-log retention deployments, such a repair may fetch exact historical logs directly from the configured provider or same-host Reth substrate without re-materializing `raw_logs`, provided it is bounded by existing `normalized_events` rows, matches the retained `(chain_id, block_hash, transaction_hash, log_index)` identity, decodes through adapter-owned logic, and updates only the documented lossy fields. The bounded row set may be served by a narrow repair-only partial index over those legacy normalized events; that index is operational lookup support and not a broad audit surface. The initial admitted repair is ENSv1 PublicResolver-compatible `TextChanged` payload repair: legacy generic `RecordChanged` rows with `record_family=text`, `record_key=text`, and `selector_key=null` may be rewritten to selector-specific `text:<key>` rows, and selector-specific text rows missing a retained value may have that value filled, when the source log verifies against the indexed key hash (upstream: .refs/ens_v1/contracts/resolvers/profiles/ITextResolver.sol:L5 @ ens_v1@91c966f) (upstream: .refs/ens_v1/contracts/resolvers/profiles/TextResolver.sol:L21 @ ens_v1@91c966f). This repair does not write `raw_*`, `backfill_*`, projections, manifests, discovery rows, execution rows, or public API state directly; projection workers must rebuild affected projections afterward.
