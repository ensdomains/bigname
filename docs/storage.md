...
| `resolver_profile_authority_journal` | storage API persists; indexer manifest/discovery orchestration advances | revision-fenced last authority/epoch snapshot whose forced resolver-profile work was durably queued |
...
| `projection_normalized_event_changes` | normalized-event storage trigger; projection workers consume | append-only, commit-ordered downstream change log for normalized-event inserts and canonicality-state updates |
...
flip. Historic stored-lineage checkpoint promotion consumes these rows
...
Stored-lineage promotion reads the chain's `discovery_admission_epochs` value,
verifies the watched set without holding the epoch row lock, then briefly locks
the row in shared mode and accepts that coverage proof only when the value is
unchanged. It releases the lock while it validates provider ancestry, fills
checkpoint ancestors, and persists raw and normalized state. Immediately
before checkpoint advancement it reacquires the shared lock, requires the same
verified epoch again, and holds the lock through the checkpoint write. That
final transaction briefly excludes lineage writers and repeats the same-height
fork check over the exact promoted path, so a competing stored branch cannot
appear between verification and persistence. Before manifest sync or discovery
admission changes any manifest, discovery edge, or
active address that can alter the watched set, its transaction acquires the
owning chain's epoch row in exclusive mode. A real watched-set mutation bumps
that epoch before the transaction commits. Such changes may commit during
either unlocked preparation window, but epoch drift then refuses checkpoint
advancement; a writer already in flight when final revalidation starts holds
the exclusive fence, and a writer which starts later waits for the checkpoint
write. The process-local
coverage frontier retains the normalized required intervals for each watched
`(source_family, address)` tuple. On an admission-epoch change it diffs the
current requirements against that snapshot: removed or shortened intervals
need no historical coverage read, while added intervals are checked from their
own explicit `active_from_block` through the already-verified stored anchor. A
retroactive admission therefore cannot inherit an advanced checkpoint, but an
unrelated watch-set change does not rescan unchanged tuples from the chain's
earliest watch. A manifest event-topic selector change invalidates the retained
proofs only for that source family. An empty memo after restart, or a checkpoint
regression below the memoized lower bound, still fails closed by verifying all
requirements no later than the earliest explicit watched `active_from_block`;
closed historical discovery intervals backed by active source and mapped
target-family manifests participate in that lower bound. Deprecated-profile
intervals remain stored audit evidence but are not coverage authority, and
unknown starts remain unknown rather than being synthesized as block zero.
Normal promotion polls extend the memoized frontier instead of rescanning full
history.
...
- Promotion coverage is the gap-free union of the exact tuple's address rows
  ...
  different source family does not. This lets independently completed default
  32-block `ops-catchup` jobs prove a 1,024-block stored-lineage promotion
  slice without weakening the exact watched-tuple requirement.
...
Manifest and discovery authority use `resolver_profile_authority_journal` rather than an in-memory before/after pair. Its singleton row stores the last fully journaled resolver-profile semantic snapshot, including targets that may subsequently be removed, the per-chain `discovery_admission_epochs` snapshot captured with it, and a monotonic revision. A full journal attempt reads the discovery epochs, loads current authority, and reads the epochs again; epoch drift during that scan discards the inconsistent capture. Revision zero is an initialization marker: the first stable capture establishes the baseline without scheduling historical absence repair, because an upgraded database's generation-one raw-log corpus is explicitly of unknown completeness. The queue migration seeds historical targets only for generation-zero corpora, whose retained history has never crossed a destructive boundary. After that baseline, the journal diffs each stable current snapshot against the persisted snapshot, then one storage transaction force-enqueues every changed target into `resolver_profile_input_changes` before compare-and-set advancing the journal revision and both snapshots. The handoff commits both changes or neither: a stale journal revision rolls back its queue increments, then reloads both snapshots and retries instead of publishing duplicate work derived from an obsolete diff. A crash after a later authority mutation but before this handoff is recovered by startup or the next epoch guard.
...
The indexer expands a dirty seed resolver to every active target whose profile derives from that seed: all current and legacy ENSv1 PublicResolver-generation seeds ripple across `ens_v1_resolver_l1`, and the Basenames `L2Resolver` seed ripples across `basenames_base_resolver`. A non-seed candidate remains address-scoped. The absence-aware adapter then reloads retained readable resolver-emitter blocks for the affected addresses, upserts observations still admitted by the current profile, and orphans prior raw-log-backed normalized events that are now absent. After that adapter work and direct projection invalidations are durable, the indexer acknowledges only the generation it loaded. A concurrent generation makes the compare-and-set fail and remains pending, so a crash or race cannot mark unseen work complete.
...
In a never-destructively-rotated, generation-zero staging corpus, the current valid closure boundary for a stateful adapter is the earliest retained canonical raw fact for that adapter/source graph. After deletion, truncation, or another destructive generation change, a retained suffix is not a closure boundary. Replay then requires a current source-family-specific, generation-bound gap-free coverage proof through the target or a documented durable versioned adapter-state snapshot anchored to that generation and input revision. The persisted retained-history proof tuple authorizes ENSv2 root/registry closure only. Target-bounded live reorg repair has a separate transient proof for ENSv1 and Basenames registry discovery: exact current-generation tuple coverage plus a discovery-admission epoch carried into the destructive writer. That transient proof does not authorize general ENSv1 replay, resolver, registrar, wrapper, authority, or permission families. Existing `normalized_events`, `surface_bindings`, `resources`, projection rows, or API-visible state are not semantic input for deterministic stateful replay and must not be used as implicit snapshots.
...
`current_projection_replay_status` rows let worker restarts resume from the first unfinished projection family instead of restarting bootstrap/full replay from the start. They are worker-owned operational progress: not API truth, not projection data, not live-readiness state, and ignored unless the recorded replay version is still current and the recorded normalized target covers the requested replay target. Automatic bootstrap holds its cross-process replay lock from apply-cursor baseline selection through family replay and creates a missing `projection_apply_cursors` row immediately before releasing that lock. If a target-covering current-version marker exists without that cursor, restart seeds the cursor at change `0` rather than acknowledging a later watermark that the completed family may not have applied. Continuous apply therefore cannot observe the handoff cursor before the protected replay has completed.

`projection_invalidations` rows are the durable key-scoped work queue for projection refreshes. `projection_normalized_event_changes` is the append-only downstream input for normalized-event inserts and canonicality-state updates; migrations install the forward log and trigger without bulk-copying historical `normalized_events`. Every insert statement acquires one shared transaction-scoped advisory lock before its `change_id` identity values are allocated and retains that lock through commit. This serializes change-log writers from allocation through commit, including normalized-event trigger writes and direct repair inserts, so a visible `change_id` watermark cannot skip an earlier in-flight change. The commit-order-fence migration takes the automatic-bootstrap replay lock, pre-locks and rewinds an existing `normalized_events_to_projection_invalidations` row, then exclusively drains change-log readers and writers before installing the fence and repeating that targeted rewind. This matches derive's cursor-then-change-log lock order. If the cursor was initially absent, a pre-cutover derive either publishes before the exclusive lock and is caught by the second rewind, or waits behind the lock with a safe zero lower bound. Deployments therefore pay one full, idempotent change-log derivation after this migration; the rewind recovers any lower-id row that a pre-fence allocation-ordered cursor may already have skipped. `projection_apply_cursors` rows track consumed `change_id` watermarks for that input. Manifest, execution, and other non-normalized-event invalidation producers write the same queue directly. The primary key is `(projection, projection_key)`; repeated invalidations for the same key update the row generation, clear retry metadata, return the row to `pending`, and release any stale claim so an older apply cannot erase newer work. Projection workers claim and apply rows in projection dependency order, then delete only the claimed generation. Claims are leases with retry recovery, so rows claimed by a stopped worker become eligible again after the retry delay rather than requiring manual queue repair. Rows that fail the same claimed generation five times are removed from the live queue and copied to `projection_invalidation_dead_letters` with `state='dead_letter'`, the failure reason, timestamps, attempt count, and original queue identity for operator inspection. Dead-letter rows are durable operational evidence, not claimable work.
...
Replay version 8 therefore forces an all-current worker replay after the
permission resource summary was introduced; accepting a version-7 completion
marker would leave that projection empty. Version 8 also retains the version-7
re-evaluation of ENSv2 fresh registrar registrations as exact-name-profile
evidence. The replay changes no raw fact, normalized event, identity row, or
projection payload in SQL; the normal worker rebuild remains the projection
write owner and re-evaluates canonicality and manifest evidence from retained
inputs. Replay versioning is a bootstrap gate, not a mixed-version writer fence.
Deployments must stop or drain every version-7 worker before any version-8 worker
starts, keep public reads drained until the version-8 replay and pending
invalidations finish, and never run the two worker versions concurrently; an
older worker can otherwise publish the superseded result after the new replay.
